...
A nightly job loads yesterday's papers, filters them, and emails a digest. The author guarded the
email with `if filtered_df: send_digest(filtered_df)`.

It ran once in development against a small sample and raised immediately:
`ValueError: The truth value of a DataFrame is ambiguous.` They looked it up, found `if not
filtered_df.empty:` in an answer, pasted it in, and it worked.

Two months later a colleague is adding a second guard, this time on a Series of scores. They write
`if scores:` from muscle memory, get the same error with `Series` in place of `DataFrame`, and paste
in `.empty` again. Then they write `if score_value:` on a single extracted value — which does not
raise, because a single value is a plain float — and it silently skips every row whose score is
exactly `0.0`.

The first two errors were the library protecting them. The third was the same misunderstanding
arriving somewhere the library could not protect them. **The error message is a feature, and the
lesson it is trying to teach is that a collection of booleans is not a boolean.**
