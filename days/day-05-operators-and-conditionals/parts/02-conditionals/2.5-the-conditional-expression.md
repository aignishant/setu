...
Two people give you directions to the same café.

The first says: "At the lights, if it's green go straight on, otherwise turn left." One sentence, one
decision, and you are already walking.

The second says: "Right, so, first — is the road open? Because if the road is open, then check
whether the market is on, and if the market is on, then see if the side gate is unlocked, and if the
side gate is unlocked…" Four sentences in you have stopped listening, and you still do not know where
the café is. Somewhere at the bottom of all that there is a "…then it's the blue door", and by the
time it arrives you have lost which condition it belongs to.

Neither person is wrong about the route. The difference is that **the shape of the sentence should
match the shape of the decision**, and the second person's did not.

Two functions land in the same code review, and they are those two people.

The first is nine lines that set one variable. It starts by setting the label to nothing, opens an
`if` and sets `"high"`, opens an `elif` and sets `"low"`, and finishes with an `else` that sets
`"unknown"`. The reviewer's comment is that the whole four-line skeleton exists only to move a value
out of a branch and into a name, and the reader has to carry that variable in their head across all
nine lines to be sure it always ends up set.

The second is a validation function five levels deep. It checks that the payload exists, then that it
has a `records` key, then that the records are a list, then that the list is not empty, and then —
indented five times, forty lines below the top of the function — it finally does the work. The `else`
branches holding the error messages are scattered at the bottom, each one separated from its own test
by everything in between. The reviewer's comment is that they cannot tell which `else` belongs to
which `if` without counting spaces.

Both comments are the directions problem. Choosing between two values is one sentence. Rejecting four
kinds of bad input is four early exits. Neither is a matter of taste; each shape makes the other's
bugs impossible.
