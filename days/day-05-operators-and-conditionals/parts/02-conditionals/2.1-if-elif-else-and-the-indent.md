...
Four instructions, one after another, and anybody can cook from it.

Now imagine the last line printed slightly to the right of the others, tucked underneath "fry for
five minutes" the way a sub-step is. Suddenly it reads as *part of* the frying rather than something
you do after it, and you tip the tomatoes in at the start. The words did not change. Their position
on the card did, and the position was carrying the instruction.

Everyone has misread a recipe this way at least once. Python made that reading the rule.

Here is what it costs. A support engineer is looking at a routine that has started letting bad
records through. It walks a batch of readings and, for each one, adds a complaint to a list when the
value is out of range. The list is returned at the end.

The code is four lines and looks completely fine. The last line — the `return` — sits one level too
far to the right. It is *inside* the loop rather than after it, so the function stops after examining
the **first** record, and every record behind it has never been looked at.

Nothing raised. The function returned a list, exactly as documented. The list was simply the answer
to a smaller question than anybody had asked. Every test passed, because every test batch had one bad
record and it happened to be the first.

In most languages a closing brace would have made this visible on the page. In Python the brace *is*
the indentation, which means moving whitespace changes meaning — and it also means that when the
indentation is right, the structure cannot lie about the logic, because there is nothing else for it
to say.
