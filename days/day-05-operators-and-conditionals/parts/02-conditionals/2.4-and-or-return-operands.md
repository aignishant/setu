...
A function returns a display name. The author wrote:
...
Then the product adds a feature: users can deliberately clear their nickname to be shown by their
full name. The clearing sets it to the empty string. Which is falsy. Which the `or` chain skips —
exactly as intended, by accident.

Six months later the same idiom appears in a different file, this time on a numeric setting:

```text
retries = config.get("retries") or 3
```

A user sets `retries` to `0`, meaning "do not retry, fail fast". `0` is falsy. They get three
retries, which on a non-idempotent write means the operation happens three times.

And in a third place, a debugging session takes an hour because somebody logged the result of an
`and`:
...
It printed `0` instead of `False`, and the person reading the log spent forty minutes looking for a
count.

Three incidents, one fact: **`and` and `or` do not return booleans.** They return one of the operands
you gave them, and everything else follows.
