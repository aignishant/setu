...
That single "no" can be hiding three completely different situations. The fridge is genuinely empty.
Or the fridge is full, but only of things nobody counts as dinner. Or nobody actually looked, and
"no" is a guess.

If you are deciding whether to go to the shop, collapsing all three into one "no" is fine and
sensible. If you are deciding whether to call an electrician, it is a disaster, because one of those
three means the fridge stopped working and you have thrown away the only clue.

Python does the same collapsing, on purpose, and it is usually a kindness.

A pipeline has a step that writes a summary only when there is something to summarise. Somebody wrote
`if results: write_summary(results)` and it worked for months.

Then a downstream team asked for the summary to be written even when a run legitimately produced zero
rows — an empty summary is how they know the run happened at all. A junior engineer changed the
producing function to return an empty list instead of nothing in that case, expecting the check to
start passing. It did not. Both "nothing" and "an empty list" count as false here, and the summary
still was not written.

Confused, they changed the check to `if results is not None:`. Now the empty summary is written. So,
three weeks later, is a summary for a run that **failed** and returned an empty list because the job
feeding it had crashed. The report shows zero rows, and everybody reads it as a quiet day.

The real question was never "does this exist" or "is this empty". It was **"did this run succeed, and
how many rows did it produce"** — two facts that no single true-or-false check can carry. Truthiness
squashes several questions into one, and the skill is knowing when the squash is safe.
