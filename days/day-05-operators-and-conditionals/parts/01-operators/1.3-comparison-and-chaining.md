...
An analyst is filtering readings from a sensor. She wants values between two bounds and writes what
she would write on paper: `if low < reading() < high:`. It works.

A colleague reviewing the code says it looks like a mistake and rewrites it as
`if low < reading() and reading() < high:` — "clearer", he says. The filter still passes its tests.
In production, the readings start disagreeing with the raw log: values that were never in range get
through, and values that were in range get dropped.

`reading()` polls the sensor. The rewrite calls it twice, so the two halves of the condition are
comparing two *different* readings. The original form called it once. The "clearer" version was the
bug, and the tests missed it because the test used a fixed number, not a function.

Elsewhere in the same codebase, a report sorts a column of mixed values — numbers with a few `None`s
where the sensor dropped out — and dies with `'<' not supported between instances of 'NoneType' and
'int'`. The team's first instinct is that Python is being fussy. It is being the opposite: it is
refusing to invent an answer to a question that has none.
