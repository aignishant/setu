...
An athlete runs. A tap runs. A shop runs a sale. A colour runs in the wash. A bus runs every twenty
minutes. Five completely different things happen, and the word never changes. What decides the
meaning is *what you are talking about*, and no English speaker finds this confusing or thinks the
word is broken.

Symbols in Python work the same way. Open the interpreter and type three things:
...
One symbol, three answers, and none of them is arithmetic in any school sense. Gluing two words
together is not addition. Putting two lists end to end is not addition either. And yet `+` handles
all three without complaint.

Here is why that matters in practice. Someone writes a small class to hold an amount of money, and
wants `price + tax` to work. They decide it cannot — adding, they reason, is something the language
does to numbers, and their thing is not a number. So they write `price.add(tax)` everywhere instead,
and the code starts to read like a legal document.

It takes nine lines to be wrong about this. Once those nine lines exist, `price + tax` works, and
every function already written to take numbers accepts the new object too, without a single one of
them being changed.

**The symbol is a question. The thing on the left gives the answer.** That is the whole document, and
it is why `&` combines bits on two numbers and filters rows on two table columns, without either
