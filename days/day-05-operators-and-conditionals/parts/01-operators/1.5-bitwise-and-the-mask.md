...
A street of a hundred houses. You want the ones that are on the north side **and** have a garden.

Try to answer that with a single yes or no and you cannot. "Is the street on the north side?" is not
a question the street can answer — some of it is and some of it is not. The only way through is to
walk down the street with a clipboard and tick each house separately. A hundred houses, a hundred
ticks, and the answer is a *list* of ticks, not one tick.

That is the difference between two things that look identical in code.

Somebody is filtering a table of records. They want rows from 2023 that are also open access, and
they write it the way they would say it out loud:

```text
recent = papers[papers.year == 2023 and papers.open_access]
```

The error that comes back does not mention `and` at all:

```text
The truth value of a Series is ambiguous. Use a.empty, a.bool(), a.item(), a.any() or a.all().
```

They search the message, find an answer saying "use `&` instead of `and`", change one character, and
it works. They still have no idea why, so the next time they write a filter they will guess again —
this time adding brackets that are not needed, or leaving out ones that are.

There is a real explanation, it fits on one page, and it turns three memorised rules into one fact:
**`and` demands a single yes or no, and a column of ten thousand rows is not a single yes or no.
`&` is a symbol a type is allowed to define, and pandas defined it to mean "walk down the street with
a clipboard".**
