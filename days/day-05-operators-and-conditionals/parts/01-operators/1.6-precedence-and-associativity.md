...
A pricing script has been running nightly for a year. It computes a discounted total:

```text
total = base + surcharge * 1.18
```

The intent was to add the surcharge and then apply tax to the sum. What it does is apply tax to the
surcharge only. Every invoice for a year has been a little low, consistently, in a way that looks
like a rounding convention rather than a bug. It was found by an auditor, not by a test, because the
test fixture was a tax-exempt customer, whose multiplier is `1.0`.

In the same review, someone finds a permissions check:

```text
if user.is_admin or user.is_owner and resource.is_public:
```

`and` binds tighter than `or`, so this reads "admin, OR (owner AND public)". The author meant
"(admin OR owner) AND public". For a year, any administrator could read any private resource. Nothing
raised. The code even looks like English, which is precisely why nobody stopped on it.

Two bugs, no exceptions, no failing tests, one cause: **the expression Python evaluated was not the
expression the author read.** Parentheses cost two characters.
