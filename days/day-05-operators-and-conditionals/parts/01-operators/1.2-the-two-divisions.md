...
A team is paginating a list of results. Twenty-three items, ten to a page. Someone writes
`pages = total / per_page` and the page selector on the website starts offering page `2.3`.

They fix it with `int(total / per_page)`, which gives `2`, and ship. Three months later a colleague
adds a "credits owed" feature that can go negative, computes `int(owed / rate)` on `-23` and `10`,
gets `-2`, and issues a refund that is one unit short. The tests pass, because every test used
positive numbers.

The same afternoon, an on-call engineer is looking at a scheduling bug. A job is meant to run every
seven days starting from a reference date, and the code decides "is today a run day?" with a
remainder. For dates after the reference it is perfect. For dates before it, in the back-fill job,
it silently skips.

Three bugs, all in the same family, and all invisible in the tests because the tests used numbers you
would use in a tutorial. **Division in a programming language is not one operation and it does not
round the way school taught.** Getting the two operators and the sign rule straight is a
five-minute idea that prevents an entire genre of production defect.
