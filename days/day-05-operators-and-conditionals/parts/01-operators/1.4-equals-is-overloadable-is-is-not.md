...
You are in a shop holding a shirt, and you point at the window display.

"Is this the same shirt as that one?"

The assistant has room to interpret. Same design, yes. Same size? Let me check. Same colour, near
enough — the window one has faded a bit in the sun. They can answer helpfully, unhelpfully, or
generously, and any of those answers can be defended, because "the same" is a judgement.

Now ask a different question. "Is this literally the shirt that was hanging in the window ten minutes
ago?" There is exactly one true answer to that, the assistant cannot be generous about it, and if
they say yes then the window is now empty.

Python has both questions. `==` is the first one, and the object being compared **gets to answer it
however it likes**. `is` is the second one, and nothing can spin it.

Here is what happens when you forget which is which.

A test suite has started passing everything. It began with one helper — a stand-in for a service that
was not ready yet. Whoever wrote it wanted it to be forgiving, so they made it answer "yes, same" to
any comparison at all, on the grounds that assertions should not fail on details nobody cared about.

The service was finished. The stand-in stayed, because it was handy in four other tests. Six weeks
later, an assertion that should have caught a real regression compared a genuine result against the
stand-in and reported success. Nobody noticed until the release was out.

While hunting for that, somebody finds a second line in the same file:
...
It has never once assigned the default. `config` is a settings object whose `==` compares field
values and answers "no" to anything it does not recognise. The timeout has been missing in production
the entire time, and the network client has been reading a missing timeout as "wait forever".

Both bugs are the same bug. Somebody asked a question the other object was allowed to answer.
