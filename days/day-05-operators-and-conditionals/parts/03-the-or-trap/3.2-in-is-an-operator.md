...
It reads a file of paper identifiers, keeps the ones it has not seen before, and writes them out. The
core is four lines and reads perfectly:
...
On the developer's sample of five hundred rows it finishes instantly. On the first real file — sixty
thousand rows — it takes eleven minutes. On the monthly file it is still running the next morning.

Nobody wrote a nested loop. There is exactly one `for` in the file. But `seen` is a list, and
`paper_id not in seen` walks it from the start every single time, so by row fifty thousand each check
is comparing against fifty thousand strings. The loop is `n` iterations of an `n`-step scan, which is
`n²` comparisons — three and a half billion for sixty thousand rows.

The fix is one character: `seen = set()` instead of `seen = []`. The script then finishes in under a
second, and the `if` line does not change at all.

**The condition looked like a constant-time test and was a loop.** That is the thing to learn: `in`
hides its cost behind a syntax that gives no hint.
