...
You hand a prescription to a pharmacist and say, "no sugar in the syrup. Zero."

The pharmacist hears somebody who has not expressed a preference, and adds the standard amount.

You did express a preference. You expressed the strongest possible one. **Zero is an answer** — it is
just an answer that looks, to a careless listener, exactly like silence. Everything else in this
document is that mistake, written in code.

A client library reads a retry setting like this:

```text
self.retries = config.get("retries") or 3
```

It is a one-liner, it reads beautifully, and it has been in the codebase since the first commit.

A payments team integrates with it. Their write endpoint is not safe to repeat — calling it twice
charges the customer twice — so they set `retries: 0` in their configuration and move on, satisfied
that they have turned retries off.

Three weeks later a brief network wobble during a deploy causes a batch of writes to time out. Each
one retries. Three times. The reconciliation report the next morning shows a run of triple charges,
and the incident review traces the whole thing back to a single two-letter word.

`0 or 3` is `3`. The configuration was read, parsed, checked as a whole number, handed to the client
— and then thrown away by an `or`, because zero counts as false. Nothing raised. Nothing was logged.
The setting showed as `0` in every configuration dump the whole time, because the dump was printing
the configuration and the bug was in the client.
