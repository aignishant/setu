...
A helper collects warnings during processing. It looks completely ordinary:
...
The developer's first thought is that the tests are interfering — which is correct, but not for the
reason they suspect. They add a fixture to isolate them
([Day 2, 3.2](../../../day-02-quality-gate/parts/03-pytest/3.2-fixtures-and-tmp-path.md)) and it does
not help, because the shared state is not in the test file.

In production it is worse. The list grows all day. Records processed in the afternoon carry warnings
from the morning. A monitoring dashboard shows a warning count climbing steadily and never falling,
which everyone interprets as deteriorating data quality — and somebody spends two days investigating a
data problem that does not exist.

There is one object. It was created when the module was imported, and every call that did not pass
`problems` has been appending to it since.
