...
> **The source.** None of this is Python's invention. Every mainstream language and every modern
> processor follows the same published rules: the *IEEE Standard for Floating-Point Arithmetic*
> (IEEE 754-2019, DOI 10.1109/IEEESTD.2019.8766229), <https://standards.ieee.org/ieee/754/6210/>.
> When you meet a float surprise, you are meeting a clause of that standard, not a bug in Python.
...
Three friends finish a meal and the bill is 100 rupees. They split it three ways.

One hundred divided by three is 33.333333…, and the threes never stop. You cannot pay that. So they
each put in 33 and somebody adds the extra rupee, and nobody thinks about it again. Everybody has
done this. Nobody calls it a bug in division. It is simply that **one third cannot be written down
exactly in the digits we use**, so we write the closest thing we can and move on.

Now the same problem, in a shop.

Three items cost 0.10, 0.20 and 0.30. Add them up and the answer is obviously 0.60. A small program
that adds them prints this instead:

```text
0.6000000000000001
```

The first reaction is that the addition is broken. It is not. The second reaction is to round the
number before showing it, which hides the digits but does not change the stored value, so a check
that compares the total against 0.60 still fails.

Here is the connection. Your computer does not store numbers in the ten digits you write with. It
stores them in ones and zeros, and **in ones and zeros, 0.1 is a number whose digits never stop** —
exactly like one third in decimal. The machine keeps the closest value it can hold, which is a
whisker off, and adding three whisker-off numbers gives you an answer that is a whisker off.

Every language on ordinary hardware behaves this way. The people who avoid the pain are simply the
ones who learned two things early: money is not stored as a float, and floats are not compared with
`==`.
