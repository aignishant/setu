...
Imagine writing a shopping list under a strange rule: **you are not allowed to add a line to a piece
of paper.** If you want a longer list, you must take a clean sheet, copy across everything already
written, add the new item at the bottom, and throw the old sheet away.

The first item costs you nothing — there is nothing to copy. The second costs one line of copying.
The tenth costs nine. By the four hundredth item you are copying three hundred and ninety-nine lines
in order to write one word, and you are doing that for every remaining item on the list.

Nobody would agree to that rule for a shopping list. Plenty of people write it into code without
noticing, because it looks like the most ordinary loop in the world:
...
On a test file of two hundred lines it finishes instantly, so it ships. In use, the real file has
eight hundred thousand lines. The program does not crash and does not print an error. It simply never
finishes: twenty minutes later the fan is loud, the processor is busy, and no output has appeared.

The code is not wrong. It produces the right answer, eventually. What is wrong is the **price**.
Because a piece of text in Python cannot be added to, `result += text` cannot append. It must build a
whole new piece of text holding everything `result` already had plus the new bit, and point the name
at that. So the work grows with the *square* of the number of lines: two hundred is instant, eight
hundred thousand is effectively forever.

The fix takes two lines and makes it instant again. But you can only find the fix once you know why
text cannot be added to — which is the same rule that makes text safe to hand around everywhere else.
