...
Someone learning Python has come from a language where a variable is a box. You declare it with a
size, you put a value in it, and assigning to another variable copies the contents from one box into
another. That model is correct in that language and it works perfectly there.

They write Python with the same model in their head. For weeks it is fine, because for numbers and
strings the two models predict the same outcome — and numbers and strings are what beginners use.

Then they write a function that takes a list of records and, being tidy, sorts it before processing.
The caller's list comes back sorted. The caller did not ask for that and does not notice, until much
later, when a report comes out in an order that makes a subtotal wrong.

The box model says the function got a copy. The label model says the function got a second label on
the caller's list. **Python is the label model**, and the difference stayed invisible for weeks
precisely because it only shows up when something is mutated.

The fix is not a rule to memorise. It is replacing the picture. Once the picture is right, the
behaviour stops being surprising and starts being obvious.
