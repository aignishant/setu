...
Two people fill in a paper form for a wedding. The question is *how many guests are you bringing?*
The first writes **3**. The second writes **3**.

Now somebody has to work out the catering. There are two things you can do with those two forms, and
they are not the same thing:

- **Add them.** Three guests plus three guests is six guests. You need six chairs.
- **Put the papers side by side and read across.** You get "33". You need thirty-three chairs.

The ink on the page cannot tell you which one was meant. A **3** written on a form is a symbol. Six
is a count. Confusing the two is how a hall gets booked for thirty-three people who do not exist.

Every value that comes out of a form, a web page, a spreadsheet cell or a text file arrives as
symbols — as *text* — because that is all those things can hold. Somewhere downstream, somebody adds
the value to a running total.

In a language that quietly converts, `"3" + 3` produces something, and which something depends on the
language. Some give `6`. Some give `"33"`. Either way the program keeps going.

Say it gives `"33"`. The total is now text. The next addition sticks another number on the end. The
one after that does it again. By the end of the batch the "total" is a forty-character run of digits,
and it is written into a report field that happens to accept text, so nothing complains.

Nobody notices for a week. When somebody finally does, the thing they can see is a nonsense number in
a report, and the cause is a silent conversion on one line, in a different part of the system, that
left no error and no log entry.
...
It raises straight away, on the exact line, on the very first bad value, before anything downstream
exists to be corrupted. **The program stops instead of being quietly wrong**, and somebody fixes it
in a minute.

That is what people mean when they call Python strongly typed, and it is worth being able to explain
why it is a feature rather than a nuisance.
