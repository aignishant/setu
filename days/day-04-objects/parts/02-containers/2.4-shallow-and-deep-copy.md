...
Two people share a photo album on their phones.

Priya wants her own copy before she starts deleting the blurry ones, so she forwards the album to
herself. It arrives. She now has two albums in her list, which feels like a backup.

She deletes eleven blurry photos from the album she is holding, then opens the "backup" to check the
originals are safe.

They are gone from that one too.

What she forwarded was not the photos. It was **a link to the same album**. Two entries in her list,
one album behind both of them. Deleting from either deletes from the album, and there is only one
album.

Now here is the same thing in code, and it looks exactly like a working safeguard:
...
The comparison at the end reports zero differences. Every title matches. The cleaning function looks
like it is doing nothing at all.

Print statements go in. The cleaning function is definitely running, and it is definitely changing
titles. And yet `originals` shows the cleaned versions too.

`records.copy()` made a new **list**. It did not make new **records**. Both lists hold the same
dictionaries ([2.3](2.3-aliasing-two-names-one-object.md)), so `record["title"] = ...` writes into a
dictionary that is in both.

The backup copied the *shelf*, which nobody was changing, and not the *books*, which everybody was.

What makes this expensive is that it looks right. There is no error, a copy genuinely did happen, and
the word `copy` is sitting there in the source code. The only way to know is to understand how deep a
copy goes.
