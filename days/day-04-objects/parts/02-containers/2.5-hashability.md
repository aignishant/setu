...
Someone writes a cache keyed by a query and its filters:

```text
cache[query_terms] = results
```

`query_terms` is a list. It raises:

```text
TypeError: unhashable type: 'list'
```

They work around it by converting to a string: `cache[str(query_terms)] = results`. It works, and it
ships.

Two problems arrive later. The first is that `["ai", "safety"]` and `["safety", "ai"]` produce
different strings and therefore different cache entries, even though the query means the same thing.
The cache hit rate is far worse than anyone expected and nobody knows why.

The second is worse. Somewhere else, a similar cache uses a **custom object** as its key — one that
someone helpfully made mutable so the filters could be adjusted. Adjusting a filter changes the
object's value, which changes its hash, and the entry becomes unreachable: it is still in the
dictionary, occupying memory, findable by nothing. `key in cache` is `False` and
`len(cache)` still counts it.

The error at the start was the language preventing exactly the second bug, and the workaround
reintroduced a version of it.

**A `TypeError: unhashable type` is not an obstacle. It is a design question being asked at the right
moment**, and the right answer is usually "this key should be immutable", not "let me convert it to
something the dictionary will accept".
