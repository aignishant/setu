...
A team stores default settings for a report generator:

```text
DEFAULT_OPTIONS = {"format": "pdf", "include_charts": True, "sections": ["summary"]}
...
It works. Reports come out with a summary and an appendix.

Two weeks later somebody reports that a report has an appendix twice. Then three times. By the end of
the day, one report has fourteen appendices.

`DEFAULT_OPTIONS` is a module-level object created once when the module was imported.
`options = user_options or DEFAULT_OPTIONS` does not copy it — it makes `options` a **second name for
that one dictionary** ([1.2](../01-objects/1.2-names-are-labels.md)). Then
`options["sections"].append(...)` mutates the list inside it, permanently, for the lifetime of the
process.

Every call that does not supply options appends to the same list. The count grows with the number of
requests since the last restart, which is why it looked fine in testing (one call), fine in staging
(a few calls) and absurd in production.

The tell that nobody spotted: **`DEFAULT_OPTIONS` is named like a constant and is not one.** Nothing
in Python makes it constant; capital letters are a convention that the interpreter ignores entirely.
