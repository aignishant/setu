...
You are looking for one word in a book. There are two ways to do it.

The first is to start at page one and read until you find it. The second is to turn to the index at
the back, find the word, and jump straight to the page it names.

For a twenty-page pamphlet, both work and neither feels slow. For a nine-hundred-page textbook, one
takes a second and the other takes your evening — and the difference is not that you got faster. It
is that the index does not care how long the book is. It sends you to one place.

That is the entire difference between two of today's containers, and it is worth several seconds a
run in real code.

Here is where it shows up. A list of names has duplicates in it, and you want each name once. The
obvious version is the one almost everybody writes first:
...
It reads perfectly. On a sample of two hundred names it runs instantly, so it looks finished.

On ten thousand names it takes about four seconds. On a hundred thousand it does not finish while
anyone is willing to wait.

The reason is the word `in`. Asking `name not in seen` when `seen` is a **list** is reading the book
from page one: Python checks the entries one at a time until it finds a match or runs out. As `seen`
grows, every later check costs more, so the total work grows with the square of the input.
...
A **set** answers `in` the way an index does. It does one calculation on the name and looks in one
place. It does not matter whether the set holds ten names or ten million — the check costs about the
same, and a hundred thousand names finish in a fraction of a second.

Same logic, same output, a different container. Four seconds against instant today, and the
difference between minutes and never once the data grows.
