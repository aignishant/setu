...
A pipeline ranks papers by score and takes the top five.

```text
def top_five(papers):
    ranked = papers.sort(key=lambda p: p["score"], reverse=True)
    return ranked[:5]
```
...
The developer stares at it. `papers.sort()` clearly sorted something — there is no error on that line.
So why is `ranked` nothing?

Because `list.sort()` sorts **in place** and returns `None`. It changed `papers` — the caller's list,
incidentally ([1.2](../01-objects/1.2-names-are-labels.md)) — and handed back nothing at all. The
name `ranked` is bound to `None`, and slicing `None` raises.

They change it to `sorted(papers, ...)` and it works. Two things are now true that were not before:
the function returns the top five, **and** the caller's list is no longer being reordered behind their
back.

This is a five-second fix once you know the rule, and a twenty-minute confusion when you do not — and
the traceback is genuinely unhelpful, because it names the *symptom* (`None` cannot be sliced) rather
than the cause (a mutating method returned nothing).
