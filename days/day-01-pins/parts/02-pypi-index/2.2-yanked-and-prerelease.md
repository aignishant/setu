...
true, which is why this part exists rather than being a footnote in [1.2](../01-versions/1.2-version-specifiers.md).
...
[1.1](../01-versions/1.1-semantic-versioning.md)'s `Version(s).is_prerelease` is how you filter it.
...
- `[Version(v) for v in doc['versions']]` — parse each one, so comparisons are numeric rather than
  alphabetical ([1.1](../01-versions/1.1-semantic-versioning.md)'s `'3.10' > '3.9'` trap).
...
mature pattern for a major bump — [1.1](../01-versions/1.1-semantic-versioning.md)'s subject — is to test
