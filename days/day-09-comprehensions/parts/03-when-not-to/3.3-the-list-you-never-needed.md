...
A lorry pulls up and you need two numbers: how many boxes there are, and what the whole load weighs.

One way is to unload everything onto the warehouse floor, then count the boxes and weigh the pile.
This works, and it needs floor space for the entire delivery.

The other way is to weigh each box as it comes off the tailgate, adding as you go and making a tally
mark. This also works, and it needs no floor space at all — just a set of scales and a pencil.

For a small van it makes no difference. For a full lorry, only one of them is possible, and the
person who insists on the floor method finds out at exactly the wrong moment.

Here is the floor method. An overnight aggregation, three lines, and a memory profile that grew with
the data until it no longer fitted:
...
Twenty million lines, and twenty million numbers held all at once so that two numbers could be worked
out from them. The list existed for the duration of two function calls and was then rubbish.
...
or, keeping the one-sentence shape:

```text
total = sum(len(line) for line in handle)
```

— and the count then needs either a second pass over the file or the loop above, because the boxes
only come off the tailgate once
([2.3](../02-dict-set-gen/2.3-the-generator-expression.md)).

That last constraint is the real content of this part. Being lazy is not free: it trades memory for
**one use only**, and knowing when you genuinely need a second pass is the difference between a
correct optimisation and a silently empty result.
