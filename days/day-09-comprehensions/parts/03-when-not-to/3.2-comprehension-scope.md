...
Somebody borrows your pen to sign a form.

If they hand it back, nothing about your desk has changed. If they slip it into their pocket without
thinking, then an hour later reach for "their" pen and find yours, two people are mildly confused and
one form gets signed in the wrong colour.

Neither behaviour is wrong on its own. What causes trouble is not knowing which one you agreed to.

Two bugs, and the second one only exists because the first was fixed.

The first is old. In Python 2, the variable a comprehension used for counting was left behind
afterwards:

```text
x = "important"
squares = [x * x for x in range(5)]
print(x)          # 4, in Python 2 - the variable had been overwritten
```

A comprehension could quietly destroy a variable belonging to the function around it. Python 3 gave
comprehensions a space of their own and the bug went away — the pen always comes back.
...
and a reviewer points out that it calls `normalise` twice for every title
...
One call, correct — and `key` now **still exists after the comprehension has finished**, holding
whatever it held last. A later line in the same function that happens to use `key` for something else
quietly reads a leftover.

The isolation was added on purpose. The walrus was given a way out of it, also on purpose. Both
decisions are right, and knowing which one is in force on a given line is what stops either of them
surprising you.
