...
A recipe, compressed onto a card so it fits in the kitchen drawer:

> Fry the onions, add the tomatoes, simmer, serve.

Perfectly usable. It is also not the recipe. The full version said *"if the onions catch, throw them
out and start again"*, and *"taste for salt at this stage, not at the end"*, and *"this takes longer
at altitude"*. Those three sentences were not padding. Each one is there because somebody once ruined
the dish, and the card has quietly thrown all three away.

The card is not shorter than the recipe. It is the recipe with the hard-won warnings deleted.

Here is that card. A pipeline function, turned into a single expression by somebody who had just
finished this day's first two sections:
...
It works. It replaced fourteen lines. In review, three separate people ask what
`for score in [compute_score(r)]` is doing — it is a way of holding a temporary value inside a
comprehension, which is a real technique, and here it is doing the job the walrus operator does more
honestly ([3.2](3.2-comprehension-scope.md)).

The fourteen lines it replaced contained: a count of rows rejected for each of three separate
reasons, a log line for records whose score could not be computed, and a `try` around `normalise`
because one feed occasionally sends a malformed title.

**All three are gone.** Not because the author decided to remove them — because a comprehension has
nowhere to put them, so turning the code into one meant deleting them. The refactor did not compress
fourteen lines into one. It deleted eleven lines of hard-won operational detail and kept three.
