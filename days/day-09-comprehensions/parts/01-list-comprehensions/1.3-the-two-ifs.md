...
A teacher marks a test where wrong answers lose marks, so a few students end up below zero. Two rules
get discussed in the staff room, and they sound almost the same:

1. **"Anyone below zero counts as zero."**
2. **"Ignore anyone below zero."**

Rule one keeps every student in the class. Thirty students go in, thirty come out, and a few of them
have a zero.

Rule two takes those students out of the class altogether. Thirty go in and twenty-six come out.

Now somebody works out the class average by adding up the marks and dividing by **thirty**, because
thirty is the number of students in the register and always has been. Under rule one that is right.
Under rule two it is wrong — quietly, plausibly wrong, by exactly the proportion of students who were
removed — and nobody will ever look at 58% and think *that number is missing four people*.

Here is that staff-room conversation as one line, changed once.

```text
scores = [s if s > 0 else 0 for s in raw_scores]
```

Every raw score is kept and negatives become zero. Two thousand rows in, two thousand rows out.
Correct for the requirement, which was "clamp negatives at zero".
...
Also correct — and the code downstream computes a mean by dividing the sum by a row count **stored
earlier in the pipeline**. The list is now shorter than that count. The mean is quietly too low, and
it stays wrong for a quarter, because 0.71 is every bit as believable as 0.83.

The two lines differ by moving five characters. One keeps the length and one does not, and **the type
checker, the linter and the tests all see two perfectly valid comprehensions**. The only thing that
catches it is knowing these are different operations, and asserting the length.
