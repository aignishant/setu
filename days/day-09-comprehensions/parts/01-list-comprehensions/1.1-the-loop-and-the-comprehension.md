...
The plan names this day's example: *the same transform written as a loop and a comprehension; read both
aloud.*
...
Read the first aloud: *"titles is an empty list; for each paper, append the stripped title to titles."*
Four nouns, one of which — `titles` — appears three times and does nothing but accumulate.

Read the second: *"titles is the stripped title of each paper."*

That is the whole difference, and it is not brevity. The loop describes **a procedure that ends with
the answer**; the comprehension describes **the answer**. A reader of the loop has to simulate it to
know what `titles` contains at the end. A reader of the comprehension does not.

Which is also exactly why a comprehension stops being the right tool the moment the body does more than
produce one value per item — because then there is no single sentence to read.
