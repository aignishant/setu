...
A flattening, written twice by the same person on the same afternoon.

```text
flat = [item for row in rows for item in row]
```

Correct. Then a colleague asks for a matrix built from two ranges, and the same person writes:
...
wrote the loops in the order the *expression* mentions them rather than the order they should run.

The rule is one sentence and it is worth memorising rather than deriving: **write the `for` clauses in
exactly the order you would write the nested loops.** The expression's variable order is irrelevant.

The same afternoon produces the third bug, which is quieter. Flattening a list of rows where one row is
`None`:
...
The filter that fixes it goes **after the clause it guards**, not at the end — and getting that wrong
gives a `TypeError` in one position and silently wrong output in another.
