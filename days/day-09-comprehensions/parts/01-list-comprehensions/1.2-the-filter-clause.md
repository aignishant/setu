...
A parser converting a column of strings to integers.

```text
years = [int(value) for value in column]
```

It runs for months. Then a row arrives where the year is `""` — a blank cell from a spreadsheet export
— and the whole job dies with `ValueError: invalid literal for int() with base 10: ''`.

The fix somebody reaches for first is a `try`. Comprehensions do not take statements
([1.1](1.1-the-loop-and-the-comprehension.md)), so that means abandoning the comprehension and writing
a loop, which is what happens: eight lines, a `try`, a `continue`, and a counter that nobody added.
...
Blank cells produce nothing. The expression never sees them, because **the filter runs first**.

And the sting: the fixed version silently drops rows. Two hundred blank cells become two hundred
missing years, and the report's total is quietly lower. **A filter is a decision to discard**, and a
filter that nobody counted is a data loss nobody logged — which is why the loop version, with its
counter, was not entirely wrong.
