...
Two ways of counting people into a hall.

The first: let everybody in, seat them, and then count the occupied seats. It works, and it needs a
hall with enough seats for everybody who turns up.

The second: stand at the door with a clicker and press it as each person walks past. It works too,
and it needs no hall at all — just you and the clicker. Ten people or ten thousand, your hands hold
the same amount.

The clicker has one catch, and it is worth being clear about. Once somebody has walked past, they
have walked past. If you are asked afterwards how many were wearing coats, you cannot go back and
look. The seated version can be counted again as many times as you like.

Here is that choice, one character apart. A log analyser:

```text
total = sum([len(line) for line in open(path)])
```

On the 5 MB sample: instant. On the two-gigabyte real log: `Killed`, exit code 137, and no Python
traceback at all
([Day 6, 4.1](../../../day-06-loops/parts/04-loop-cost/4.1-the-accidental-quadratic.md)).

The comprehension seated all twenty million numbers before `sum` looked at a single one. Remove two
characters:
...
Flat memory, the same answer, and it finishes.

The same week brings the opposite mistake. Somebody writes a helper that "returns the parsed rows":
...
A caller does `rows = parse(path)`, asks for `len(rows)` and gets a `TypeError`, switches to
`sum(1 for _ in rows)`, gets the count — and then goes to work through `rows` and gets **nothing at
all**. No error. Everybody had already walked past the clicker.

**One character decides whether you are holding a hall or a clicker**, and the two are good at
opposite things. Knowing which one is in your hand is the whole subject.
