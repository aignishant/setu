...
A gym with a wall of lockers, and a member of staff writing names on them with a marker.

R. Kumar joins, and gets locker 12. His name goes on the front.

Six months later a different R. Kumar joins. The assistant checks the wall, sees "R. Kumar" already
written on locker 12, and — being tidy — writes the new member's details against that locker. There
is now one entry for R. Kumar in the system, one person happily using locker 12, and one person whose
locker quietly stopped being theirs.

Nothing was refused. Nobody was told two people shared a name. The wall looks perfectly organised,
and the membership list is one person short of the truth.

Here is that wall as a lookup table, built from a list of records:
...
Three weeks later a reconciliation shows 9,847 records in the index and 10,000 in the source. Nothing
raised, nothing was logged, and no code changed.

The source has 153 duplicate identifiers — versions of the same record arriving from two feeds. The
comprehension built the index perfectly and **kept the last one it saw of each identifier**, throwing
away the earlier ones. It is a dictionary, keys are unique, and that is exactly what a dictionary
does.

The bug is not the comprehension. The bug is that **a de-duplication happened and nobody decided it
should**. Which copy survives — the first or the last — is a question about the data, not about
Python ([Day 8, 3.2](../../../day-08-containers/parts/03-dedup/3.2-order-preserving-dedup.md)), and
here the later feed carried *less* information, so the index quietly lost the author lists for 153
records.

**Building a lookup table from a key that is not unique is a de-duplication.** The line is right;
what is missing is the count and the decision.
