...
At the end of a wedding somebody asks how many different countries the guests travelled from.

You go round the room with a scrap of paper, writing down each country as it comes up and skipping
any you already have. At the end you count the lines and say, with complete confidence, "eleven
countries."

Then somebody asks which country the *first* guest came from.

You have no idea. You never wrote that down. Your scrap of paper is a jumble of eleven names in
whatever order you happened to meet them, and even that order got muddled when you turned the page.
The method answered the question you asked and destroyed the information you did not ask about, and
that was not a mistake — it was the method.

Here is that scrap of paper. A count of distinct authors across a batch of records:
...
Then two things happen in the same fortnight.

The first: somebody changes `paper["authors"]` from a list of names to a list of small records. The
comprehension raises `TypeError: unhashable type: 'dict'` — loud, immediate, and obvious to fix once
you have read it.
...
because the number Python computes for a piece of text differs between processes. Comparing today's
report against yesterday's is unreadable, and it takes a week for somebody to work out that nothing
changed except the order.

**A set comprehension buys you de-duplication and charges you order.** That is a fine trade when you
wanted a count, and the wrong one the moment the result is shown to a person.
