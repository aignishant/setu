...
tell an average over 10 000 requests from an average over three. Add the count, and convert both to plain
Python numbers on the way out — `np.float64` slips through the JSON encoder because it subclasses
`float`, but the `np.int64` count will not, and I would rather not have that distinction load-bearing."*
