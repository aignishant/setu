...
|---|---|---|
| [1.1 Semantic versioning, and what a major bump promises](parts/01/1.1-semantic-versioning.md) | What is a version number actually claiming, and who enforces it? | `foundation` |
| [1.2 Version specifiers, and what each one costs](parts/01/1.2-version-specifiers.md) | Why is `==` right in a service and harmful in a library? | `foundation` |
| [1.3 The resolution problem](parts/01/1.3-the-resolution-problem.md) | Why can adding one package change five others, or fail outright? | `working` |

### Section 2 — reading the truth from the index

| Part | What it answers | Level |
|---|---|---|
| [2.1 PyPI's JSON API](parts/02/2.1-the-pypi-json-api.md) | Where does a version number come from, if not a tutorial? | `working` |
| [2.2 Yanked releases and pre-releases](parts/02/2.2-yanked-and-prerelease.md) | When is "latest" the wrong answer — and why does `==` install withdrawn releases? | `production` |
| [2.3 `scripts/check_pins.py`](parts/02/2.3-the-check-pins-script.md) | What turns a one-off command into a tool you still run in Phase 27? | `working` |

### Section 3 — freezing it

| Part | What it answers | Level |
|---|---|---|
| [3.1 The Python-version intersection](parts/03/3.1-the-python-version-intersection.md) | Is 3.12 still the right interpreter — computed, not inherited? | `production` |
| [3.2 Freezing — `uv lock`, `--locked`, `--frozen`](parts/03/3.2-freezing-into-pyproject-and-the-lock.md) | How do you prove intent and resolution still agree, and move one pin alone? | `working` |
| [3.3 Regenerating `docs/PINS_DS.md` from evidence](parts/03/3.3-regenerating-pins-from-evidence.md) | Why must a document of facts be generated rather than written? | `working` |

### Section 4 — keeping it frozen

| Part | What it answers | Level |
|---|---|---|
| [4.1 The three breaking changes already in this stack](parts/04/4.1-the-three-breaking-changes.md) | Why is a *silent* breaking change more expensive than a loud one? | `production` |
| [4.2 Drift, the freshness check, and the amendment protocol](parts/04/4.2-drift-and-the-amendment-protocol.md) | Principle 13 detects, Principle 14 responds — why does neither work alone? | `production` |
...
|---|---|---|
| `urllib.request`, `json` | standard library | [2.1](parts/02/2.1-the-pypi-json-api.md) |
| `tomllib` | standard library since 3.11 | [1.2](parts/01/1.2-version-specifiers.md) |
| `packaging` | already installed, transitively | [1.1](parts/01/1.1-semantic-versioning.md) |
...
[2.3](parts/02/2.3-the-check-pins-script.md): per-package error isolation, a cache with an expiry, a
...
([3.3](parts/03/3.3-regenerating-pins-from-evidence.md)). If your computed intersection from
[3.1](parts/03/3.1-the-python-version-intersection.md) disagrees with what that section claims, do
not quietly edit it: that is an amendment ([4.2](parts/04/4.2-drift-and-the-amendment-protocol.md)).
...
| Caching | `.pins-cache.json`, expiry from [2.3](parts/02/2.3-the-check-pins-script.md) — a re-run should make **no** requests |
...
([2.1](parts/02/2.1-the-pypi-json-api.md)).
...
- **Comparing version strings as strings.** `'3.10' > '3.9'` is `False`. Sorting versions
  alphabetically works until a project reaches `.10` — [1.1](parts/01/1.1-semantic-versioning.md).
- **Misreading `~=`.** `~=3.0` lets the *minor* move; `~=3.0.5` does not. Same operator, decided by
  whether you typed a third number — [1.2](parts/01/1.2-version-specifiers.md).
- **Assuming `--frozen` verifies something.** It skips the check. `--locked` is the one that refuses
  — [3.2](parts/03/3.2-freezing-into-pyproject-and-the-lock.md).
- **Taking `info.version` as "safe to pin".** It does not promise to exclude yanked releases, and
  your `==` pin is exactly the form that installs them — [2.2](parts/02/2.2-yanked-and-prerelease.md).
- **Pinning a release candidate** because your script sorted versions without filtering pre-releases
  — [2.2](parts/02/2.2-yanked-and-prerelease.md).
...
[2.1](parts/02/2.1-the-pypi-json-api.md), [2.2](parts/02/2.2-yanked-and-prerelease.md) and
[3.2](parts/03/3.2-freezing-into-pyproject-and-the-lock.md) come from them — re-check before you rely
