...
prev: "../02/2.3-the-check-pins-script.md"
...
([2.1](../02/2.1-the-pypi-json-api.md)) — a specifier
([1.2](../01/1.2-version-specifiers.md)) constraining the **interpreter** rather than another
...
package ([1.3](../01/1.3-the-resolution-problem.md)): there is one interpreter. So the set of legal
...
- **[Day 0's part 1.4](../../../day-00-setup/parts/01/1.4-python-3-12-under-uv.md) asserted 3.12 with
...
- The request is the one from [2.1](../02/2.1-the-pypi-json-api.md), unchanged — same endpoint, same
...
- `try/except ... continue` — per-package failure isolation from
  [2.3](../02/2.3-the-check-pins-script.md). A broad `except Exception` is acceptable in a reporting
...
- `combined.contains(Version(c))` — test each candidate interpreter. Note `.contains()` here rather
  than the `in` operator used in [1.2](../01/1.2-version-specifiers.md); both work, and `.contains()`
...
disk is a separate fact ([Day 0, 2.5](../../../day-00-setup/parts/02/2.5-the-venv-you-never-activate.md)).
...
resolver does across the whole package graph ([1.3](../01/1.3-the-resolution-problem.md)). Doing it
...
[4.2](../04/4.2-drift-and-the-amendment-protocol.md) — you have found a change that amends the plan.
