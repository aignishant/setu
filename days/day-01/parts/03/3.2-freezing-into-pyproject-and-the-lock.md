...
[Day 0's part 2.4](../../../day-00-setup/parts/02/2.4-uv-init-pyproject-and-the-lockfile.md)
...
[Day 0, 4.2](../../../day-00-setup/parts/04/4.2-the-first-commit.md) insisted on reading
...
- **Principle 13's weekly freshness check produces drift** ([2.3](../02/2.3-the-check-pins-script.md)).
...
- `echo "exit status: $?"` — the answer that matters. Zero means they agree. This is the same
  ...
  ([Day 0, 3.3](../../../day-00-setup/parts/03/3.3-the-done-gate.md)), and it is what makes the check
...
- Note the escaped `\"` — the shell argument is double-quoted, so inner double quotes need escaping
  ([1.2](../01/1.2-version-specifiers.md)). Another reason real code lives in a file.
...
- `${PIPESTATUS[0]}` — the exit status of the **first** command in a pipeline, not the last. `$?`
  ...
  problem from [Day 0, 3.1](../../../day-00-setup/parts/03/3.1-set-euo-pipefail.md), and
...
- `--upgrade-package ruff==0.17.0` would move it to a specific version instead of the newest
  permitted one, which is what you use after [2.3](../02/2.3-the-check-pins-script.md) has told you
...
([Day 0, 4.2](../../../day-00-setup/parts/04/4.2-the-first-commit.md)), and it is common enough that
...
have not evaluated. `uv tree` before and after ([1.3](../01/1.3-the-resolution-problem.md)) makes that
...
[1.1](../01/1.1-semantic-versioning.md)'s table encoded as automation policy.
...
([Day 0, 2.4](../../../day-00-setup/parts/02/2.4-uv-init-pyproject-and-the-lockfile.md)). That is what
...
([Day 0, 2.4](../../../day-00-setup/parts/02/2.4-uv-init-pyproject-and-the-lockfile.md) mentioned
