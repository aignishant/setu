...
next: "../03/3.1-the-python-version-intersection.md"
---
...
[2.2](2.2-yanked-and-prerelease.md) filters yanked and pre-release, [1.1](../01/1.1-semantic-versioning.md)
...
([Day 0, 3.3](../../../day-00-setup/parts/03/3.3-the-done-gate.md)). That single number is what lets
...
on a project you do not have. `tomllib` from [1.2](../01/1.2-version-specifiers.md) reads the real
source.
...
- **`docs/PINS_DS.md` is regenerated from this tool's output**
  ([3.3](../03/3.3-regenerating-pins-from-evidence.md)) — so its machine-readable mode is not a
...
- `doc['project'].get('dependencies', [])` — runtime dependencies, defaulting to empty so a project
  without any does not raise ([1.2](../01/1.2-version-specifiers.md)).
...
- `r'^([A-Za-z0-9._-]+)\s*==\s*([^\s;]+)'` — a raw string (`r'...'`, so backslashes are literal)
  ...
  which is where an environment marker begins ([1.2](../01/1.2-version-specifiers.md)).
...
- `CACHE = pathlib.Path('.pins-cache.json')` — a dotted filename, and one you must add to
  `.gitignore` ([Day 0, 2.2](../../../day-00-setup/parts/02/2.2-gitignore-before-secrets-exist.md)):
...
- `encoding='utf-8'` on both sides — never rely on the platform default
  ([1.2](../01/1.2-version-specifiers.md)).
...
an amendment protocol ([4.2](../04/4.2-drift-and-the-amendment-protocol.md)). The professional skill
...
[3.3](../03/3.3-regenerating-pins-from-evidence.md) without any of them parsing human prose. This is
why `git status --porcelain` exists
([Day 0, 4.2](../../../day-00-setup/parts/04/4.2-the-first-commit.md)) — a stable machine format
...
**Next:** [3.1 — The Python-version intersection, computed rather than assumed](../03/3.1-the-python-version-intersection.md)
