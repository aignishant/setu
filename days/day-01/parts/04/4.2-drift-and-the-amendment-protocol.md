...
world moved. Today you froze a set of versions ([3.2](../03/3.2-freezing-into-pyproject-and-the-lock.md))
and generated a document recording them ([3.3](../03/3.3-regenerating-pins-from-evidence.md)). Both
...
pin, plus the MCP specification page. The tool from [2.3](../02/2.3-the-check-pins-script.md) produces
...
| **Major** (`3.0.5 → 4.0.0`) | removals ([1.1](../01/1.1-semantic-versioning.md)) | **stop.** Write an addendum before any code changes |
| **Yanked** ([2.2](../02/2.2-yanked-and-prerelease.md)) | the release was withdrawn | move immediately; your `==` pin is still installing it |
| **Python floor/ceiling moved** ([3.1](../03/3.1-the-python-version-intersection.md)) | your interpreter window changed | addendum — this affects every future day |
...
- **Principle 13 is a standing commitment for 240 days.** It only survives if it is cheap
  ([2.3](../02/2.3-the-check-pins-script.md)) and if its output has an obvious next action — which is
  the table above.
...
- The tool from [2.3](../02/2.3-the-check-pins-script.md), whose exit status is the answer: zero means
...
- `|| echo` — the fallback for before you have built it, so the command is honest about its
  ...
  ([Day 0, 2.5](../../../day-00-setup/parts/02/2.5-the-venv-you-never-activate.md)).
...
- `classify` is a **pure function** — inputs in, verdict out, no network and no files. That is
  ...
  separation of *fetching* from *deciding* that [2.3](../02/2.3-the-check-pins-script.md) argued for.
- `Version(pinned)`, `Version(current)` — parsed, never compared as strings
  ([1.1](../01/1.1-semantic-versioning.md)).
- `if c < p` — the current version is *older* than your pin. That is not ordinary drift: either your
  ...
  ([2.2](../02/2.2-yanked-and-prerelease.md)).
- The order of the checks matters: equality first, then the anomaly, then major, then minor, then
  ...
  `except` clauses ([2.3](../02/2.3-the-check-pins-script.md)).
...
- `sed -n '/start/,/end/p'` — the pattern-range address from
  [3.3](../03/3.3-regenerating-pins-from-evidence.md): print from the first match to the next.
...
- The heredoc from [Day 0, 2.2](../../../day-00-setup/parts/02/2.2-gitignore-before-secrets-exist.md),
...
for 240 days ([2.3](../02/2.3-the-check-pins-script.md)'s note on alert fatigue). If you find silence
...
[1.1](../01/1.1-semantic-versioning.md) — without reading what it removed or recording the decision.
...
yanked ([2.2](../02/2.2-yanked-and-prerelease.md)). Your `==` pin is still installing the withdrawn
...
[2.3](../02/2.3-the-check-pins-script.md)'s tool, and so are the failure modes: a check that is too
...
The same argument as the gate in [Day 0, 3.3](../../../day-00-setup/parts/03/3.3-the-done-gate.md).
