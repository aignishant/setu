...
([2.3](../02/2.3-git-init-and-what-a-repo-is.md) explains why it cannot simply be deleted). The config
...
You already have the model from [2.3](../02/2.3-git-init-and-what-a-repo-is.md): three places — the
...
from [1.2](../01/1.2-git-and-git-bash.md), and the message.
...
is part of history — [2.3](../02/2.3-git-init-and-what-a-repo-is.md) established why that cannot be
quietly undone, and [2.2](../02/2.2-gitignore-before-secrets-exist.md) established what it costs when
...
can commit safely in [3.3](../03/3.3-the-done-gate.md), and it is the fastest way to answer "is there
...
- **Principle 1: no commit, no day.** This is the command that makes a day exist, and you will run it
  roughly 240 times.
- **`./m done N` runs `git add -A` for you** ([3.3](../03/3.3-the-done-gate.md)). It stages
  everything, without looking, which is safe *only* because
  [2.2](../02/2.2-gitignore-before-secrets-exist.md) was written first — and because you looked.
- **`docs/TRACKER.md` and `./m status` read commit messages** to report progress
  ([3.3](../03/3.3-the-done-gate.md)). The `day-NN: complete` convention is what makes that possible.
...
- **What you must not see:** `.env`, anything under `.venv/`, or anything under `data/`. If any of
  ...
  [2.2](../02/2.2-gitignore-before-secrets-exist.md) is the part to return to, and the fix is much
...
- `day-00:` — the convention `./m status` greps for ([3.3](../03/3.3-the-done-gate.md)). Zero-padded,
  so `day-04` sorts next to `day-05` — the same reason `pad()` exists in
  [3.2](../03/3.2-the-case-dispatcher.md).
...
**What it means.** The `git config` commands from [1.2](../01/1.2-git-and-git-bash.md) were skipped.
...
[2.2](../02/2.2-gitignore-before-secrets-exist.md):
...
([2.3](../02/2.3-git-init-and-what-a-repo-is.md)). The two commands differ by one flag and by whether
your work survives.
...
stops here rather than printing success ([3.3](../03/3.3-the-done-gate.md)). It is a refusal, not a
failure.
...
([2.3](../02/2.3-git-init-and-what-a-repo-is.md)). On your own unpublished branch this is routine and
