...
[1.1](../01/1.1-why-one-tool-owns-the-environment.md) said the environment needs.
...
first, because of the `PATH` rule from [1.1](../01/1.1-why-one-tool-owns-the-environment.md) — first
match wins.
...
- A script you run, an editor's test runner ([1.5](../01/1.5-the-editor-and-the-interpreter-trap.md)),
...
- **`./m check` and `./m done` are shell scripts** ([3.2](../03/3.2-the-case-dispatcher.md)). A script
...
- **Day 2 runs the same commands in GitHub Actions.** There is no interactive shell there and nobody
  ...
  Git Bash in [1.2](../01/1.2-git-and-git-bash.md).
...
- `ls -A .venv` — the layout from the table above. On Windows you will see `Scripts/`; on
  macOS/Linux, `bin/`. This difference is why [1.5](../01/1.5-the-editor-and-the-interpreter-trap.md)'s
...
- `sys.executable` — from [1.1](../01/1.1-why-one-tool-owns-the-environment.md): the interpreter
  actually running. It should now point **inside** `.venv`, not at the standalone 3.12 from
  [1.4](../01/1.4-python-3-12-under-uv.md) and not at any system Python.
...
- `|| source .venv/bin/activate` — `||` runs the right side only if the left **failed**, the mirror of
  `&&` from [1.3](../01/1.3-uv-the-one-binary.md). Together they make one line work on both platforms.
...
Python, which is the [1.1](../01/1.1-why-one-tool-owns-the-environment.md) rule holding inside the
...
**Next:** [3.1 — `set -euo pipefail`, the four characters that make bash safe](../03/3.1-set-euo-pipefail.md)
