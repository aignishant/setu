...
- `cat > .gitignore <<'EOF' … EOF` — the heredoc from [1.5](../01/1.5-the-editor-and-the-interpreter-trap.md):
...
- `;` versus `&&` — a semicolon runs the next command **regardless** of whether the previous one
  succeeded, unlike `&&` from [1.3](../01/1.3-uv-the-one-binary.md). Used here because `git init` in an
