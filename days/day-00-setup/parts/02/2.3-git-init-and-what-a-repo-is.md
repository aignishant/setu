...
- **`./m done N` commits automatically** ([3.3](../03/3.3-the-done-gate.md)). When it refuses, or
...
- You will see `objects/` (the content-addressed database), `refs/` (branches and tags — the files
  ...
  [1.2](../01/1.2-git-and-git-bash.md)), and `hooks/` (scripts Git can run at certain moments).
...
- `|` — the pipe from [1.2](../01/1.2-git-and-git-bash.md): feed it to the next command's input.
...
Same shape as the format-on-save argument in [1.5](../01/1.5-the-editor-and-the-interpreter-trap.md),
