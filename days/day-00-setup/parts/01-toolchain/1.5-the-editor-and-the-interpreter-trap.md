...
([2.5](../02-skeleton/2.5-the-venv-you-never-activate.md)), most editors find it automatically. The important part
...
- **Ruff** — the same linter and formatter that `./m check` runs from
  [3.1](../03-m-script/3.1-set-euo-pipefail.md) onward, shown live in the editor. Its value is that the editor and
...
- `.venv/Scripts/python.exe` — the Windows layout. **On macOS and Linux this path is
  ...
  [2.4](../02-skeleton/2.4-uv-init-pyproject-and-the-lockfile.md), and the setting simply starts working then.
- `python.terminal.activateEnvironment: false` — stop the editor from auto-activating the environment
  ...
  ([2.5](../02-skeleton/2.5-the-venv-you-never-activate.md)), so auto-activation adds hidden state that can disagree
...
**Next:** [2.1 — The folder skeleton, and why each folder exists](../02-skeleton/2.1-the-folder-skeleton.md)
