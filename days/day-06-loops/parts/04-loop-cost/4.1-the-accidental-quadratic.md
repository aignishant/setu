...
A nightly report has been getting slower.

Six months ago it took forty seconds. Three months ago, four minutes. Last week it did not finish
before the morning, and the team started it manually at nine and watched it run until lunch.

Nobody changed the code. The data grew — roughly four times over six months, which is entirely normal
growth — and the report went from forty seconds to four hours. That is a factor of three hundred and
sixty for a factor of four in data.

The profiler points at one line. It is a membership test against a list that has been accumulating
inside the loop:
...
scans it, so by the halfway point every check is walking hundreds of thousands of entries, and the
total is proportional to `n²`.

The fix is `processed = set()`. The report goes back to under a minute, and it will stay there when
the data grows again — because the shape of the growth changed, not just the constant.

**The difference between forty seconds and four hours was one word**, and nothing in the source
suggested a nested loop.
