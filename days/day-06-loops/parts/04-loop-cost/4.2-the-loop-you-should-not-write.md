...
Counting a thick stack of banknotes.

You can count them one at a time, and you will be right. It is not the wrong method, and there is
nothing clumsy about it. But a bank has a machine that riffles the whole stack in three seconds, and
no amount of counting faster will catch up with it — because the machine is not counting faster, it
is doing the job a completely different way.

That is the difference between the last document and this one.

An engineer has just fixed a quadratic. The de-duplication that took four hours takes fifty seconds,
the report runs at night again, and everybody is happy.

Two months later it takes four minutes. The data grew, and this time the growth is honest — fifty
seconds to four minutes is roughly proportional, which is exactly what was promised. There is no bug
here at all.

They profile it anyway. The slowest section is a loop that multiplies two columns of numbers together
and adds up the results, over eight million rows. It is a perfectly good loop with a fixed cost per
row. It takes three minutes.

The same calculation, handed to NumPy as `(a * b).sum()`, takes forty milliseconds. Not because the
method changed — it is the same multiply-and-add — but because the Python version does eight million
separate interpreter steps, eight million lookups on eight million wrapped-up number objects, and
eight million bookkeeping updates, while NumPy makes one call into compiled code that walks a solid
block of memory.

**The quadratic was a bug. This is not a bug; it is what the interpreter costs.** And the fix is not
