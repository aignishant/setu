...
A board game. Every turn you move one square, and the winner is whoever reaches square thirty first.

There is a card in the deck that says **miss a go**. Draw it, and you do not move. That is fine and
even fun, as long as most cards are not that card.

Now shuffle a deck that is nothing but "miss a go". Everybody keeps taking turns. Everybody keeps
drawing. Nobody ever moves, and nobody notices anything is wrong, because turns are still happening
and the game *feels* like it is in progress. It will never end.

Here is that deck, written by somebody who had just learned `continue`:
...
The intention is obvious and reasonable. When the service says "you are going too fast", wait a
moment and try again without counting it as one of the three real attempts.

What happens is that `continue` jumps straight back to the top, skipping `attempts += 1` on the way.
If the service is telling you to slow down — which is exactly when this branch runs — `attempts`
never goes up, `attempts < 3` stays true forever, and the loop calls the service every two seconds
until somebody kills it. Against a free tier with a daily allowance, that is the entire day's budget
spent in an afternoon, by a loop that logged nothing.

The same `continue`, in a `for` loop over a fixed list, would have been perfectly safe, because a
`for` loop moves forward no matter what the body does. **The danger is not `continue`. It is
`continue` in a loop whose only forward movement lives in the body.**
