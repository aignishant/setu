...
You have lost your keys, so you go through the house drawer by drawer. The plan in your head has two
halves, and you say the second half out loud without ever thinking about it:

> "Check every drawer. If I find them, stop. **And if I get all the way to the end without stopping,
> then they are not in the house — go and check my coat.**"

That second half is a real, useful idea, and everybody has it. It is not "otherwise". It is
specifically *I finished the whole search and never stopped early*, which is a different thing from
*the drawers were empty* and a different thing again from *I found them*.

Python has that idea. Unfortunately, it spelled it `else`.
...
The reviewer says the `else` is wrong. "That fires when `mirrors` is empty, but it also *looks* like
it fires when the loop runs. An `else` on a `for` reads as 'otherwise', and there is nothing here for
it to be otherwise to."

The author says it is correct and has a precise meaning: the `else` runs only when the loop was
**not** broken out of, which is exactly the keys-not-in-the-house case.

They are both right about something. The behaviour is exactly what the author says it is. And the
reviewer's objection is also real, because everywhere else in the language `else` pairs with a
condition sitting right above it, and here it pairs with a `break` that might be twenty lines away.

The team lands where most teams land: **use it, and put a comment on it.** The behaviour is worth
having — the alternative is a flag variable, which brings back the problem from
