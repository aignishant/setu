...
A row of numbered chairs, and your job is to take out the broken ones.

You start at chair one and work along. Chair three is broken, so you pull it out — and everybody
sitting beyond it shuffles up one seat, because that is what people do. You carry on to what is now
chair four. But the person who *was* in chair four moved into chair three while you were lifting, and
you have already walked past chair three. You never looked at them.

Do that along a row of two hundred and you will miss roughly one person for every chair you removed,
and you will be certain you checked everybody.

Here is the same walk, in code:
...
On the test fixture — one blank row among five — it works perfectly. On the real batch it leaves
blank rows behind, and how many it leaves depends on where they were. Two blanks next to each other:
one survives. Three in a row: two survive. Blanks scattered singly: all removed, and the code looks
correct.

The team's first theory is that `remove` is failing. It is not. The loop keeps a position number.
When you delete position 2, everything after it shifts down by one — the item that was at 3 is now at
2 — and the loop, about to move on to 3, steps straight over it.

The second theory is that the fix is to use a `while`. It is not. The fix is to stop changing the
thing you are walking along: **build a new list, or walk a copy.**

The dictionary version of this mistake is kinder. It raises `RuntimeError: dictionary changed size
during iteration` and stops on the spot. The list version says nothing at all, and that difference is
why list bugs of this kind reach production and dictionary ones do not.
