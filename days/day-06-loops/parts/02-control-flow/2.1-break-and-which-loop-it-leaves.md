...
You are in a meeting room, inside an office, inside a building. Somebody says "leave."

Leave what? Walking out of the meeting room puts you in the corridor, not on the street. And saying
"leave" a second time, from the corridor, does not put you on the street either — it just leaves the
corridor. There is no word that means "leave all three at once"; you have to name what you are
leaving, or keep going one at a time.

Python's `break` is exactly this word, and it means "leave *this* loop" — nothing more.

A matching job pairs incoming records against a reference list. For each incoming record it scans the
references for a match, and stops scanning as soon as it finds one:
...
It works. Then a new requirement arrives: stop the whole job after the first match, because the job
is now a smoke test that only has to prove the pipeline is connected. Somebody adds a second `break`
underneath the first, reasons that two `break`s must leave two loops, and ships it.

The job does not stop. The second `break` is never reached — the first one already left the inner
loop — and the outer loop carries on through all forty thousand incoming records, doing nothing.

The reviewer's question is the one worth keeping: **`break` is not "stop", it is "leave this loop"**,
which loop is decided entirely by indentation, and there is no way to say "leave two". The fix is not
another `break`. It is a flag, or a function with a `return`, or — usually best — rearranging things
so there is only one loop to leave.
