...
That is a completely sensible thing to say. It is also, if the kettle is not switched on at the wall,
a sentence you could be standing there fulfilling tomorrow morning — because "it has not boiled yet"
and "it is never going to boil" look exactly the same from where you are standing. Nothing in your
plan distinguishes them, so nothing in your plan ever ends.

Here is that plan in code:
...
It works for a year. Then an export fails on the server, and `is_done()` starts answering `False`
forever, because a failed job is not a finished job. The script asks every five seconds, for eleven
days, until somebody spots the process on a monitoring dashboard. It never logged anything, because
there was nothing to log — it was doing exactly what it had been told.

In the incident review, somebody points out that the loop has no upper limit, no timeout, and no
branch for "the thing I am waiting for has failed". Somebody else points out something worse: the
same shape appears in four other places in the codebase, and each of them was written by copying this
one.

A `while` loop is the only thing in Python that can run forever with nothing wrong on the page. The
habit that prevents it is not cleverness. It is **writing the way out before you write the body**.
