...
A delivery arrives, and you check it against the order.

You put the order slip on the left and the boxes on the right, and you go down them together. Item
one, box one. Item two, box two. Item three, box three. Everything matches, so you sign for it and
the driver leaves.

There were five items on the order slip. There were three boxes. You stopped when you ran out of
boxes, because that is what "go down them together" means when one side is shorter, and every pair
you actually checked *did* match. So you signed for a complete delivery that was missing two boxes,
and you were not being careless — the method itself stopped early and did not mention it.

Here is that in code, and it cost a model evaluation.

The script has a list of predictions and a list of true answers, and walks them together with
`for pred, actual in zip(predictions, labels):`, counting matches. It reports an accuracy of 0.94,
the team is pleased, and the model ships.

A month later somebody notices the evaluation ran on 8,000 rows when the test set has 10,000. The
predictions list was short — a batch had failed quietly further upstream and returned fewer rows than
it was given — and `zip` stopped at 8,000. No error. No warning. No mention of a length mismatch.
The accuracy was real. It was the accuracy on the first 80% of the test set, and the missing 20% were
the hard cases from the end of a sorted file.

The fix is one keyword. `zip(predictions, labels, strict=True)` raises
...
**A tool that quietly gives you a smaller answer that looks right is more dangerous than one that
stops.** That is the whole lesson, and `zip`'s default is the wrong way round for data work.
