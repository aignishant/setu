...
You are reading a long book. You read a page, turn it, read the next one, and carry on until there
are no pages left.

Notice what you never did. You never photocopied all nine hundred pages and spread them across the
desk before starting. You never counted the pages. You did not need a bigger desk for a longer book,
because at any moment there is exactly **one page in front of you**, and the book knows how to hand
you the next one.

That is the difference between two ways of getting through a large file, and it is the difference
between a program that works and a program the operating system kills.

A team is processing server logs. The file is two gigabytes.

The first version reads it with `lines = open(path).readlines()` and loops over `lines`. That is the
photocopier: `readlines()` builds a list holding every line in the file before the loop starts. It
works on the developer's 5 MB sample and dies on the real file.

Somebody suggests looping over the file itself: `for line in open(path):`. It looks like it should be
worse — surely a file is not a list, so how can you go through it? — and it is the version that
works. Memory use stays flat at a few kilobytes no matter how big the file gets.

The team's picture was "a `for` loop walks a collection". The true picture is "a `for` loop asks
something for the next item, over and over, until it says there are none left", and a file can hand
over the next line without ever knowing how many lines it has.

Once that picture is right, streaming, generators, database cursors and the endless sequences of Day
11 stop being four separate topics and become one.
