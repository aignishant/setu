...
Someone is estimating memory for a simulation. They need to loop over ten million steps and they know
a Python integer is roughly twenty-eight bytes, so they budget about three hundred megabytes for the
loop and worry about it.

A colleague tells them to check. `sys.getsizeof(range(10_000_000))` reports **48 bytes**. The same as
`range(10)`. The same as `range(10**18)`, which could not fit in the memory of every computer ever
built.

`range` does not contain numbers. It contains a start, a stop and a step, and it computes each value
when asked. The loop from [1.1](1.1-what-a-for-loop-actually-does.md) never needs more than one at a
time, so nothing is ever stored.

The same person, in the same week, writes `for i in range(1, len(rows)):` to "skip the header" and
loses the last row of every file — because they added one to the start and forgot that the stop was
already exclusive. Two facts about the same object: one saves three hundred megabytes, and the other
costs a row per file forever.
