...
Every retry loop in this curriculum is the same twelve lines.

On Day 3 they wrapped a `429` from a free-tier model endpoint. On Day 95 they will wrap a training
epoch that has not converged. On Day 155 they will wrap an embedding call. On Day 182 they will wrap
an agent's think-act-observe cycle, and on Day 223 there will be a hundred of them running at once and
the cap will be the only thing standing between a demo and a bill.

The plan says so explicitly — the capped loop is one of the shapes it names as recurring across the
whole 240 days, from `PY-05` here to `ML-06`'s gradient descent to `AGT-01`'s agent loop to `MAS-10`'s
runaway caps.

So it is worth building once, by hand, understanding every line — rather than reaching for a library
decorator whose defaults you have not read. That is Principle 2: from scratch before library. The
library version is better and you should use it in production; you should also be able to write this
