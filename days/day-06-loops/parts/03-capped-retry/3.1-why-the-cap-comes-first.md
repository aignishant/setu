...
You call a friend and it rings out. You call again a minute later. Completely normal.

Now imagine you set the phone to redial automatically, forever, and their phone is switched off. Your
battery dies, their voicemail fills up, and neither of those was your intention — you just never said
how many times to stop at, because in your head you were only ever going to try twice.

Now imagine a hundred people doing that at the same moment, and your friend turns their phone back
on.

That last picture is a real and common kind of outage, and it has a name.
...
do next. Each of them retries. None of them stops. When the search service comes back up, it is met
by eleven services' worth of stored-up retries arriving at once — many times normal traffic — and it
falls over again within seconds. It comes up, gets knocked down, comes up, gets knocked down.

The pattern is called a **retry storm**. The retries did not cause the original failure. They
prevented the recovery. And every one of those retry loops was written by a competent person who
meant to add a limit later.

Somewhere in the same estate a smaller version happens weekly and nobody notices: a nightly job
retrying a request that is malformed. It will never succeed — the message is invalid, and repeating
an invalid message does not make it valid — so the loop runs until the scheduler kills the job six
hours later, having sent two hundred thousand identical failing requests.

**A retry is a bet that the failure is temporary.** The limit is your statement of how much you are
