...
A caching layer keyed on a query.

```text
cache = {}
def search(terms, year):
    key = terms + [year]          # terms is a list of strings
    if key in cache:
        return cache[key]
    ...
```

It raises immediately: `TypeError: unhashable type: 'list'`. The developer's fix is
`key = str(terms) + str(year)`, which works — and then a colleague notices the cache is missing
constantly. `["neural", "attention"]` and `["attention", "neural"]` are the same query and produce
different keys, so every reordering is a cache miss and a wasted model call.

The second fix is `key = tuple(sorted(terms)) + (year,)`. Now the key is hashable, order-independent
within the terms, and — the part that matters — **it says what it is**: a fixed-shape record of
(sorted terms, year), not a growable list that happens to be used as an identifier.

The `TypeError` was the language asking a design question. *Is this a collection of things you will
add to and remove from, or is it one composite value?* The first fix answered "collection, coerced
into a string". The second answered "composite value", which was true all along.
