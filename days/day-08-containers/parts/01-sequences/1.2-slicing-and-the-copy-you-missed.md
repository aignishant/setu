...
Your phone has a contact list, and you make a second list — a "favourites" list — by adding some of
the same people to it.

You now have two lists. You do not have two copies of anybody. Open your friend's card from the
favourites list, change their phone number, and the number has changed in the main list too, because
there was only one card the whole time and both lists were pointing at it.

Everybody understands this about phones and nobody finds it surprising. The same arrangement in code
catches people constantly, because the word `copy` is right there and the word makes a promise the
operation does not keep.

A pipeline takes a batch of records, keeps the originals for an audit log, and tidies a working copy:
...
The audit log contains the **tidied** titles. Every one of them. The copy that was taken before the
change shows the state after it.

The developer's picture was that `[:]` copies the data. It copies the *list* — a new set of pointers
— and both lists point at the same records. Changing a record through one list is visible through the
other, because there is only one record.

Six weeks later, the mirror image of the same mistake, in the same file. Somebody reads
[Day 6, 2.4](../../../day-06-loops/parts/02-control-flow/2.4-mutating-while-iterating.md), learns
that you must not remove from a list you are walking along, and writes `for r in records[:]:` — which
is **correct**, because there the shallowness does not matter at all: the loop only needs a list of
positions that will hold still, and the records being shared is beside the point.

Same operation, two situations, one right and one wrong. The rule is not "slicing is dangerous". It
is **know which level you copied.**
