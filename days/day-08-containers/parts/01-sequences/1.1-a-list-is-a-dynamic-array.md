...
A queue, written the obvious way.
...
It is correct. On the developer's fifty-job sample it is instant. On the production batch of two
hundred thousand jobs it takes eleven minutes before the first job is processed — not to run, to
*start* — and the whole run takes hours.

`pop(0)` removes the first element. A list stores its elements in one contiguous block, so removing
the front means moving every remaining element one slot left: 199,999 pointer copies for the first
job, 199,998 for the second. Two hundred thousand jobs is twenty billion pointer moves.

The fix is one import and one word: `collections.deque` and `popleft()`. Both ends become constant
time and the job finishes in seconds.

The list did nothing wrong. It is an array, arrays are fast at the end and slow at the front, and the
whole of this part is learning which operations are which so that the choice is deliberate rather than
accidental.
