...
> **The source.** `list.sort()` is timsort, and timsort is a written design with a known
> bug in its history — taught in full in
> [the timsort note](../../papers/01-the-timsort-note.md).
...
A leaderboard, sorted by score, descending.

```text
papers = papers.sort(key=lambda p: -p["score"])
```

`papers` is now `None`. The next line raises `TypeError: 'NoneType' object is not iterable`, which is
at least loud.

Fixed to `papers.sort(key=lambda p: -p["score"])`, and the leaderboard works — until a batch arrives
where two papers have the same score, and the tie is broken differently on every run. The report is
non-deterministic: two runs over the same data produce different orderings, and a diff of yesterday's
output against today's is noise.

The team's theory is that Python's sort is unstable. It is not — it is famously stable. The
non-determinism comes from **upstream**: the papers arrive from a set, and set iteration order depends
on hash values and insertion history
([2.1](../02-sets-and-dicts/2.1-a-set-is-a-hash-table.md)). The sort faithfully preserved the input
order among equal elements, and the input order was arbitrary.

The fix is a **total** sort key: `key=lambda p: (-p["score"], p["title"])`. Ties are broken by title,
which is deterministic, and the report becomes diffable.

Three bugs — a `None`, a non-deterministic tie, and a misdiagnosis of stability — and all three are in
this document.
