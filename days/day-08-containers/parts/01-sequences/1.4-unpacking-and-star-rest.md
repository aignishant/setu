...
A form with three boxes on it: name, date, signature.

Somebody hands you a page with four things written on it. If your habit is "first thing into box one,
second into box two, third into box three", you fill in the form, it looks complete, and **the fourth
thing quietly disappears** — you never wrote down that it existed, and nobody downstream will ever
know it was there.

If instead your habit is "this page should have exactly three things on it", you look up and say
"there are four things here". That is a two-second conversation, and it happens on the day the page
changed, not eight months later.

Here is that page as a file. A parser written against three columns:
...
job stops on the first row: `ValueError: too many values to unpack (expected 3)`.

The on-call engineer's fix is to take the three by position instead. It works. It also **silently
ignores** the new column, which is fine, and silently ignores a fifth and a sixth, which is less
fine, and — the part that matters — silently accepts a line with only two fields, then raises an
`IndexError` three lines later somewhere completely different.

The version that says what it means is `doi, title, year, *extra = line.split(",")`. It takes the
three fields it needs, gathers anything past them into `extra`, and still stops immediately on a line
with fewer than three — which is the behaviour that made the original version correct in the first
place.

**Unpacking is not a shorthand for indexing.** It is you declaring the shape you expect, and the
