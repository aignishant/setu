...
The plan names this day's example: *de-duplicating 10 000 arXiv IDs in O(n) instead of O(n²) — timed,
both ways.*

The measurement is the point. Two engineers can both "know" that a set is faster and behave completely
differently in a code review: one accepts a list-based dedup on a fixture of fifty because it is
readable, the other rejects it because they have watched the numbers move.

The difference is not knowledge, it is calibration. When you have personally run the same four lines at
2,000, 8,000 and 32,000 items and watched one column go 0.04 → 0.6 → 9.5 seconds while the other goes
0.001 → 0.004 → 0.016, the phrase "it will be fine, the list is small" stops being persuasive — because
you know what "small" turns into when the data grows by the factor it grows by every year.

This part is that measurement, done properly: three sizes, the ratio rather than the seconds, and the
projection to the size the data will actually be.
