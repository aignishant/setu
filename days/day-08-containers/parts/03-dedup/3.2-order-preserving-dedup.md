...
You have the year's top twenty songs written down in order, and a couple of them appear twice because
whoever typed the list was not paying attention.

You put the page through a machine that removes duplicates. It hands back a page with no duplicates
on it — and the songs in a completely arbitrary order.

The duplicates are gone. So is the chart. You asked for one thing and lost another, and the machine
did nothing wrong: nobody ever told it the order mattered, and removing duplicates is not a job that
has anything to do with order.
...
It works, and the report is wrong every day in a different way. A set destroys the ranking, so the
"top 20" is twenty arbitrary records out of the full list — and because set order depends on computed
hash values that differ between runs
([2.1](../02-sets-and-dicts/2.1-a-set-is-a-hash-table.md)), it is twenty *different* arbitrary
records each time.

The developer's fix is `sorted(set(ranked_papers))[:20]`, which is at least repeatable and sorts
alphabetically by title — still not the ranking.

The correct fix is one word: `list(dict.fromkeys(ranked_papers))[:20]`. Duplicates removed, one pass
through the data, and the order is the order the records arrived in, which is the ranking.

The lesson goes well past de-duplication. **A set answers "which distinct values are here". It does
not answer "which distinct values are here, in order".** Reaching for it because it is the fast
container, and losing a property you needed on the way, is the most common container mistake there
is — and the fix is almost always a dictionary, because a dictionary is a set that also remembers.
...
**Next:** that is the last document of Day 8.
