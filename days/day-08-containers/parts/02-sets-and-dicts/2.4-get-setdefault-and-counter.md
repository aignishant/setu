...
A wall of pigeonholes in an office, and four people who sort the post.

The first checks whether a pigeonhole exists for the name on the envelope, makes one if it does not,
and drops the letter in. Careful, slightly slow, always right.

The second has a clever habit: reaching for a pigeonhole *creates* one if it is missing. Brilliant
for sorting post. Ruinous when somebody wanders over and asks "is there anything for Priya?" —
because the act of looking makes an empty pigeonhole for Priya, and by the end of the month the wall
is covered in empty boxes for people who never received a letter.

The third does not sort post at all; they keep a tally on a sheet, one mark per letter per person.

The fourth has a habit that costs the office money. Their rule is "if there is no pigeonhole, put the
expensive courier package in it" — and to follow that rule, they **fetch the courier package first,
every single time**, before checking whether the pigeonhole already exists. The pigeonholes are
correct. The courier bill is enormous.

All four of those are in one codebase. The first:
...
Correct, four lines, three lookups per record.

The second is `defaultdict(list)`, one line inside the loop, and the source of a bug two weeks later:
a report used to ask `if year in by_year` to check whether a year had any records, somebody changed
it to `if by_year[year]`, and **the mere act of asking created an empty entry**. The report now lists
every year anybody ever queried, each with zero records, and the totals stop reconciling.

The third counts with `by_year[year] = by_year.get(year, 0) + 1`, which is correct and is `Counter`
...
`expensive_lookup` runs on **every call**, including every cache hit, because arguments are worked
out before the method is entered. The cache is perfectly correct and saves nothing at all.

Five tools, four jobs, and the differences between them are exactly the sort of thing that produces
