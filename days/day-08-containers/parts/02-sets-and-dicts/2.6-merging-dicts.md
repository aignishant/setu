...
You order the set meal, which comes with rice, dal, a vegetable and a sweet.

"Can I have paneer instead of the vegetable?"

A good waiter swaps one dish and brings you the other three. A literal-minded one hears *instead of
the set meal's contents: paneer*, and brings you a plate with paneer on it and nothing else. They did
what you said. They just took "instead of" to apply one level higher than you meant it, and now you
have no rice.

Everybody has had an order go wrong this way, and the annoying part is that nothing was refused and
nothing was reported. The food simply arrived with things missing.

Here is the same order, as a configuration system with three layers — packaged defaults, a site file,
...
The merged configuration's `database` section is `{"host": "db.internal"}`. **The port, the timeout
and the pool size are gone** — not overridden, *absent*. The site file replaced the whole database
section rather than changing one thing inside it.

Nothing raises. The port falls back to the driver's own default, which happens to be 5432, so it
works. The timeout falls back to *no timeout at all*, and eighteen months later a network problem
leaves connections hanging until somebody restarts the process.

The merge did exactly what it says on the tin: for the key `"database"`, the later value wins. It is
a **shallow** merge, and so is every other spelling of it.
