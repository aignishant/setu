...
> **The source.** The "one step, whatever the size" claim in this part holds because Python's text
> hash is a keyed cryptographic function, chosen in *PEP 456 — Secure and Interchangeable Hash
> Algorithm* (2013), <https://peps.python.org/pep-0456/>. That decision is also why `hash('setu')`
> gives a different number in every process.
...
The coat check at a wedding.

You hand over your coat and get a numbered ticket. The attendant reads the number, walks straight to
that hook, and hangs the coat. Three hours later you hand back the ticket and they walk straight to
the same hook. It takes the same few seconds whether there are twenty coats or two thousand, because
**the ticket says where the coat is** — nobody has to look through the rail.

Now ask the attendant a different question: "which coat came in third?"

They have no idea. They never recorded arrivals. The coats are arranged by ticket number, which has
nothing to do with when anybody arrived, and there is no way to work it out from the rail.

That is not a flaw in the coat check. It is the same fact seen from the other side: the reason
finding a coat is instant is that position is decided by the ticket, and the reason arrival order is
unknowable is *also* that position is decided by the ticket. You cannot keep one and drop the other.

Two facts arrive in a team on the same afternoon, and they are both this.

The first is a fix. A de-duplication that took four hours takes fifty seconds when `seen = []`
becomes `seen = set()`
([Day 6, 4.1](../../../day-06-loops/parts/04-loop-cost/4.1-the-accidental-quadratic.md)). Everybody is
pleased and nobody asks why.

The second is a bug. A report that lists categories prints them in a different order on every run.
The categories come out of a set. Somebody assumes sets are sorted — they are not — then assumes they
are in the order things were added — they are not that either — and eventually discovers that the
number Python computes for a piece of text is deliberately different in every process, so the same
set comes out in a different order on two runs of the same script.

That last detail is not an accident, and it is not Python being awkward. Text hashing was made
unpredictable on purpose by *PEP 456 — Secure and Interchangeable Hash Algorithm* (2013),
<https://peps.python.org/pep-0456/>, to stop an attacker from choosing inputs that all land on the
same hook and turning every lookup back into a walk down the rail.

Both facts come from one sentence: **a set is a coat check.** Membership is fast because the value
tells you where to look. Order is arbitrary because "where to look" is a computed number, not a
position in a queue. The speed and the disorder are the same property, seen from two sides.
