...
It works. A colleague, reviewing, says `present` should be `set(row.keys())` because "`keys()` returns
a list-like thing and you cannot do set arithmetic on it". The author says it already works. Both are
half right: `keys()` is not a list and not a set — it is a **view**, and views support set operations
precisely so this line reads the way it does.
...
`before` includes `checked_at`. It is not a snapshot; it is a window onto the live dict, and the dict
changed underneath it. The audit log records the state after the change, every time.

**The same property makes the first line elegant and the second line wrong.** A view is cheap because
it holds no data — and holding no data is exactly why it cannot remember anything.
