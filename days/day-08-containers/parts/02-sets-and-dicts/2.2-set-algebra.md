...
Two lists after a wedding: the people who said they were coming, and the people who actually turned
up.

Everything anybody wants to know is one short sentence:

- Who said they were coming and did not turn up?
- Who turned up without saying?
- Who is on both lists?
- Who is on either list at all?

Nobody answers those by reading the first list and then scanning the whole second list for each name.
You would put the two lists side by side and go down them together, and the whole thing takes a
minute.

Here is somebody doing it the other way, in code. A weekly reconciliation between two systems:
...
Nine lines, two nested loops, a flag and a `break`. It is correct. On forty thousand identifiers each
side it takes six minutes, which is annoying but survivable once a week.
...
It runs in under a second, and — the argument the reviewer actually makes — **it says what it
means.** "The database ids that are not in the index" is a sentence, and `-` is that sentence.

The same file has three more of these questions, each written out as its own nested loop: which are
in both, which are in either, which are in exactly one. All four become one line each, and the file
loses thirty lines.
