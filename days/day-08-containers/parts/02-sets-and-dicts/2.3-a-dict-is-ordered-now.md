...
Somebody at the airport is printing their boarding pass at a kiosk, carefully, twice, because "they
don't let you use your phone."

That was true. It was true for years, and they learned it properly and it saved them trouble at the
time. It stopped being true a long while ago, and now the printing is a small ritual that costs five
minutes and achieves nothing. Nothing goes wrong. They are simply doing extra work for a reason that
expired.

Code does this too, and it is one of the harder things to spot in a review, because the code *works*.

A configuration merger, written by somebody who learned Python in 2015:
...
The reviewer asks why it is not an ordinary dictionary. The author says dictionaries do not keep
things in order.

That was true. In Python 3.6 it quietly became true anyway, as a side effect of a memory improvement,
and in **3.7 it became a promise the language makes**. Since then an ordinary `dict` keeps keys in
the order they were added, and `OrderedDict` is left holding only the two things it still does that a
plain dictionary does not.

The same afternoon, a different file: a de-duplication that keeps first-seen order, written as a loop
with a `seen` set and a result list — eight lines. The reviewer replaces it with
`list(dict.fromkeys(items))`, which works precisely *because* of that promise, and which is one line.
...
as against what merely happens to be true today, is the difference between a habit and an
understanding.
