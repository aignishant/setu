...
A ticket is printed as one long strip, and the rules for reading it are written on the wall behind
the counter: the first ten characters are the booking code, the next forty are the passenger name,
the eight after that are the date.

Somebody hands you a strip that got cut short. You cut where the wall told you to cut, and the last
piece comes back **blank**.

Blank is the dangerous outcome. A blank piece of paper does not look like an accident. It looks like
a field that was left empty, which happens all the time, so you file it and move on — and nothing
anywhere records that the strip was too short to have a date on it at all.

Now compare that with a rule that said "the fifty-eighth character". A strip that short has no
fifty-eighth character, and asking for one is an obvious, loud, immediate problem.

Here is the printed strip as a program. A parser reads fixed-width records from an old export:
...
Then an upstream system starts writing shortened lines — a bug on their side, one field missing. The
parser does not raise. `line[50:58]` on a 45-character line hands back empty text, the date parser
downstream turns empty text into "no date", and records with no date are filtered out of the report
as "not yet published".

Four thousand records quietly stop appearing. Nobody notices for a month, because the report still
runs, still looks right, and nobody was watching the row count.

Had the code asked for a single character — `line[57]` — it would have raised `IndexError` on the
first bad line and somebody would have fixed the upstream bug that afternoon. **The slice's
forgiveness was the bug.**

That is the trade this document is about. A slice that never complains is a real convenience and a
real hazard, and knowing which one you are getting is the difference.
