...
A parcel arrives addressed to **JosÃ© GarcÃ­a**.

The person who ordered it is called José García, and they typed their name correctly. Somewhere
between the shop's website and the label printer at the depot, a machine read their name using the
wrong rulebook — it looked at the same marks on the wire and decided they meant different letters.
Everybody has seen this on a receipt, a menu or a badly imported spreadsheet, and everybody's first
reaction is that the computer has garbled it.

The computer did not garble anything. It faithfully applied a rulebook. It was simply not the
rulebook the name was written with, and **nothing in the parcel said which rulebook to use**.

Here is that in a job that has been running fine for months. One morning it stops on a single row:
...
tool that writes an older rulebook called Windows-1252 rather than the modern one called UTF-8. In
Windows-1252, the single byte `0xe9` means `é`. In UTF-8, `é` is written as two bytes together, so a
lone `0xe9` is not a valid UTF-8 anything, and the reader stops rather than guess.

Somebody fixes it with `errors="ignore"`. The job runs. The title in the database is now
*"tude d'un modle"*, quietly, and that paper never matches a search again.

Somebody else fixes it with `errors="replace"`. Now it is *"?tude d'un mod?le"* — at least visible,
and still wrong.

The real fix is one word: `encoding="cp1252"` on the file that is actually written in that rulebook.
But to get there you have to know the thing the error message assumes you already know — **that text
and the bytes a file is made of are two different kinds of thing, and a file only ever holds bytes.**
That distinction is the whole of this document, and it is the single most useful hour anybody spends
on text.
