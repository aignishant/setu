...
You type a colon and a closing bracket into a message, and your phone turns it into a little yellow
face.

You did not ask for a yellow face. You typed two ordinary characters. Something in between decided
those two characters were an instruction rather than punctuation, and helpfully acted on it. Most of
the time that is exactly what you wanted, which is why the feature exists — and the one time you were
writing about a bracket, it is infuriating, and you go hunting for how to turn it off.

Python does the same thing inside quotation marks, with the backslash, and it does it at a moment
when you have no chance to notice.

A script on a Windows machine cannot find a file that is definitely sitting there:
...
The error names a path that looks corrupted — a doubled backslash in one place, a strange gap in
another — and the person reading it decides the error message itself is broken.

It is not. Inside quotation marks, a backslash means "the next character is an instruction". `\r`
means *go back to the start of the line*. `\n` means *new line*. `\t` means *tab*. So `\reports`
lost its `r` to a carriage return, and the rest of the path is carrying invisible characters that
were never typed.

The cruel detail is that **not all four backslashes did something**. `\d` is not an instruction, so
Python left that one alone — which is why the path is damaged unevenly and the error message reads
like nonsense. Modern Python even raises a `SyntaxWarning` on that lone `\d`, which is the
interpreter trying to help.

The fix is one character: put an `r` in front of the opening quote, and the phone stops turning your
punctuation into faces. Better still, stop building file paths out of text at all — but that is Day
16.
