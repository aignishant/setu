...
A guest list for a party, and one guest is on it four times:
...
Anybody standing at the door reads those as one person. They would not even notice they were
different, and if you pointed it out they would shrug — a stray capital, a stray space, so what.

A computer comparing them letter by letter reads four different people, and it is not being stupid.
It is being exact, and the four strings genuinely are not the same string. The gap between "the same
person" and "the same string" is where an enormous amount of real work lives.

Here is that gap costing an afternoon. A de-duplication job is finding fewer duplicates than it
should. Two rows that are obviously the same record are not matching. The titles print identically in
the terminal. Somebody copies both into a comparison and gets `False`.

The investigation turns up three separate problems in one pair of strings. One title has a trailing
space from a spreadsheet export. One has a **non-breaking space** instead of an ordinary one — it came
from a web page, it is a different character, and it looks exactly the same in every font ever made.
And one is `"MASSIVE"` while the other is `"Massive"`, which the team had already handled by lowering
one side of the comparison and, through an old copy-paste, not the other.

Then a German colleague points out a fourth. Their data has titles containing `ß`, and
`"STRASSE".lower()` gives `"strasse"` while `"Straße".lower()` gives `"straße"` — still not equal.
`casefold()` takes both to `"strasse"`, which is what a genuinely case-insensitive comparison needs.

Four bugs, one job: **tidying text is a sequence of deliberate steps, not one method call**, and every
step you skip is a whole class of duplicate you will never find.
