...
A log parser extracts the request id from a line by finding a marker.
...
Every line in the sample has the marker. In production, some lines do not — a health check, a startup
banner, a stack trace continuation. On those, `find` returns `-1`.

`-1 + 11` is `10`. So the code slices from position 10 of a line that has no request id, takes the
first whitespace-delimited token, and produces something like `"GET"` or `"level=INFO"`. It is a
string. It looks like an id. It goes into the index.

The dashboard now shows requests with ids like `"GET"` and `"Traceback"`, each apparently occurring
thousands of times. Somebody spends a day investigating a "hot endpoint" that does not exist.

**`find`'s `-1` is a valid index.** That is the whole bug: the sentinel value for "absent" is
indistinguishable, arithmetically, from a legitimate position — and negative indexing means the slice
does not even raise. `index` would have raised `ValueError` on the first bad line.
