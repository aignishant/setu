...
Somebody reads you a shopping list off a scrap of paper:

```text
rice, oil,tea, onions,
```

You have no trouble with it. Four items. The missing space after "oil" does not slow you down, and
the comma at the end does not make you think there is a fifth item called nothing.

Now write down the rule you used, precisely enough for a machine. "Cut at every comma followed by a
space" gives you three items, one of which is "oil,tea". "Cut at every comma" gives you five, the
last of which is empty. Neither rule is what you did, and you did it without thinking.

Here is the same scrap of paper in production. A field of author names arrives as
`"Vaswani, A.; Shazeer, N.; Parmar, N."`. The job splits on `"; "` and counts three. Fine.

Then a batch arrives from a different source, written `"Vaswani, A.;Shazeer, N.; Parmar, N."` —
inconsistent spacing after the semicolons, which is exactly what human-maintained data looks like.
Splitting on `"; "` finds two separators instead of three and hands back two authors, one of which is
...
Somebody fixes it by splitting on `";"` and then trimming each piece. Better. Then a row turns up
with a trailing semicolon — `"Vaswani, A.; Shazeer, N.;"` — the split produces a final empty piece,
the count is three where it should be two, and an author with no name ends up in the index.

Meanwhile, in the same file, somebody is building the display string back up:
...
silently produces empty text for an empty list, and is one line of `", ".join(authors)`.

`split` and `join` are the two methods you will use most, and both behave differently with an
argument than without one, in a way that decides whether your reading of the list is right.
