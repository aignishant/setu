...
Somebody opens a long document and uses find-and-replace to take the word "Dr." off the front of
every name.

It works on the names. It also quietly edits *"she finished her Dr. degree in Lisbon"*, and
*"see Dr.Who, series 4"*, and a footnote about a street called Dr. Ambedkar Road. Find-and-replace
does not know where the front of a name is. It only knows what the characters look like, and it
changes every single place they appear.

Everybody who has used find-and-replace on a real document has done damage this way, and usually
found out much later.

Here is the same tool used with the same confidence in code. A cleaner strips file extensions before
indexing:
...
It works on every file anybody tests it with. Months later somebody reports that a record titled
*"A Study of .pdf Rendering Artifacts"* is indexed as *"A Study of Rendering Artifacts"* — the mention
of the format has vanished out of the middle of the title, because `replace` does not care where the
match is.

Worse turns up the same afternoon. A different cleaner removes a source prefix:
...
`"2301.00001"`, which is **a perfectly valid-looking id belonging to a different record**. The row is
quietly filed against the wrong document. No error, no warning, and nothing in the output that hints
anything happened.

Both bugs are one method doing a job it was never for. `replace` substitutes everywhere. The tools
for "only at the start" and "only at the end" are `removeprefix` and `removesuffix`, and they were
added to Python in 3.9 precisely because this mistake was everywhere.
