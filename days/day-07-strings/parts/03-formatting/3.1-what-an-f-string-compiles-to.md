...
A team moves their log messages into a configuration file, so operators can change the wording without
a deploy.
...
The loader reads the string and the code does:
...
The developer's mental model was that an f-string *formats* a string containing braces. It does not.
The `f` prefix is a compile-time instruction that turns the braces **in that literal, in that source
file** into expressions. A string that arrives at run time has no braces to compile; by then the
compiling is long over.

The fix is `config["retry"].format(attempt=n, total=t, error=e)` — `str.format`, which *is* a run-time
template method. Two tools that look alike and live at different times.

The same misunderstanding causes the opposite bug in the same codebase: someone builds a SQL query
with an f-string, interpolating a user-supplied value directly into the query text, and creates an
injection vulnerability that a run-time template with parameter binding would not have had
