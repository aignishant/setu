...
A printed price list at a market stall:
...
Nothing there is wrong. Every number is the number it is meant to be. And yet the column looks
careless, `4.5` next to `3.20` makes you wonder whether one of them was typed properly, and the last
figure is impossible to read at a glance — is that a hundred thousand or a million?

A stall holder who writes `4.50`, `12.00`, `3.20` and `12,34,567` has changed nothing about the
prices and everything about whether a customer trusts the board.

Three tables in one project, printed by three different scripts, each with one of those problems.

The first pads every column by hand and falls apart the moment a value is longer than the padding —
the column runs into the next one, and every row below it shifts.

The second rounds to two decimal places and prints `0.1` where the column should read `0.10`, because
rounding gives you a **number**, and a number has no trailing zeros to show. On a financial summary,
one row looking different from its neighbours makes a reader mistrust the whole table.

The third prints a count of `1234567` with nothing to break it up, and in a meeting somebody reads it
aloud as "a hundred and twenty-three thousand" and asks a question based on a figure that is out by a
factor of ten.

All three have a one-character answer inside a format spec: `:<12`, `:.2f`, `:,`. The spec language
is about forty characters of documentation, it works identically in f-strings and `str.format`, and
it is one of the highest-value things to learn once and keep forever.
