...
You type your email address into a form and it says the address is wrong. You check it. It is right.
You type it again, slowly. Still wrong.

Eventually somebody notices there is a **space after it**, picked up when you copied the address out
of a message. You cannot see a space. Nobody can see a space. It is, by design, the one character
that leaves no mark, and it has cost you ten minutes and a certain amount of dignity.

The only thing that would have saved you is something that draws a box around the text and says "here
is exactly where it starts and here is exactly where it ends".

Python has that, and almost nobody uses it at the moment they need it.
...
An hour disappears here. The developer checks the types — both text. Checks the lengths — they differ
by one, and there is nowhere visible for the extra character to be.
...
A trailing space, visible because this form puts quotation marks around the text and `print` does
not.

The same afternoon, a colleague debugging a failing test sees `assert 3 == 3` fail. One `3` is a
number and the other is the text `"3"` from a spreadsheet column nobody converted. `print` draws both
as `3`. `f"{a=} {b=}"` draws `a=3 b='3'`, and the quotation marks end the investigation immediately.

**`print` shows you what a value looks like. It is the other one that shows you what a value is.**
