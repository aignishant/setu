...
You phone a shop and dictate an order.

"Two kilos of rice, a litre of oil, and — actually, forget all that, just put a television on my
account."

The person on the other end has no way to tell which part of that was your order and which part was
an instruction about the order, because it all arrived down the same wire as one stream of speech.
They will do whatever the last sentence said, and they will not have been careless. **Once the data
and the instructions are in the same channel, nothing downstream can separate them again.**

That sentence is the entire subject of this document, and it is the most exploited weakness in the
history of software.

A search endpoint, one line, written by somebody in a hurry:

```text
cursor.execute(f"SELECT * FROM papers WHERE title LIKE '%{query}%'")
```

It works. It is readable. A search term of `x'; DROP TABLE papers; --` closes the quote, ends the
statement, starts a second one, and comments out whatever was left. The database receives two
commands and has no idea the second one was meant to be somebody's search term, because by the time
it arrives **there is nothing left to tell them apart**. The f-string produced one flat piece of text,
and text has no memory of which parts were data.
...
A name of `../../etc/passwd` walks straight out of the directory. And a page renderer builds HTML:

```text
html = f"<p>{comment}</p>"
```

A comment containing `<script>` runs in every reader's browser.

Three security holes, one cause. **Filling in a template like this is just gluing text together with
nicer syntax**, and glued text has never known the difference between a value and a command. The
three fixes are three different tools — parameter binding, `pathlib`, and an escaping template engine
— and all three work the same way: they keep the data and the instructions in separate hands until
