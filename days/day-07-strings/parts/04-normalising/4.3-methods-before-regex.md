...
Two ways of giving the same instruction.

The first: **"take the spaces off the front and the back."** Anybody in the room understands it,
including the person who joined last week.

The second: **`^\s+|\s+$`**. This is also correct, also precise, and understood only by people who
have learned a second, denser language first. If they have not, they cannot check your work, and if
you come back to it in a year, neither can you.

When both do the same job, choosing the second one charges every future reader a translation fee, and
the fee is paid every time somebody opens the file.
...
Shorter, roughly ten times faster on the space-collapsing line, and — the reviewer's actual argument
— **each line now says what it does in words the next reader already knows.**

Two weeks later a genuine pattern arrives: pull out identifiers that look like `2301.00001`,
`2301.00001v3`, `arXiv:2301.00001`, or the older `math.GT/0309136`. Four shapes, an optional prefix,
an optional version on the end, a varying number of digits.

The team reaches for a regular expression, and this time it is the right call — a chain of
`startswith` and `partition` calls that only approximated it would be twenty lines nobody could
check. **Neither tool answers everything, and knowing where the line falls is what makes both of them
usable.**
