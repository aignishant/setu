...
There are two ways to write the letter `é`.

The first is as one character that already has the accent on it. The second is as a plain `e`
followed by a separate accent mark that is drawn on top of whatever came before it. Print them side
by side in any font that has ever existed and you cannot tell them apart — because they are not meant
to be told apart. That is the point of the second one.

They are still two different pieces of text, made of a different number of pieces, and no amount of
squinting at your screen will reveal it.

A user cannot log in. Their username is `café_dev`. They type it correctly. The lookup fails. Support
asks them to type it again, and it fails again. Eventually somebody copies the username out of the
database and out of the login form and lines them up:
...
The database holds the accented letter as one character. The login form — filled in on a Mac, where
the default keyboard produces the other spelling — holds a plain `e` followed by a combining accent.
They look identical everywhere. They are not equal, and nothing from
[2.1](../02-methods/2.1-normalising-strip-and-case.md) will make them equal: trimming, lowering and
collapsing spaces all leave both spellings exactly as they were.

The same week brings two more. A title contains `ﬁ` — the single character some PDF tools produce for
the pair "fi" — so a search for `"identification"` misses `"identiﬁcation"`. And a title pasted from
a translated page contains Cyrillic `А` and `а` standing in for Latin `A` and `a`.

Three failures, one subject. **Looking the same is not being the same**, and there is a published
answer to it: the four normalisation forms defined in *Unicode Standard Annex #15 — Unicode
Normalization Forms* (2023), <https://www.unicode.org/reports/tr15/>. That annex is what
`unicodedata.normalize` implements, and two of its four forms deliberately throw information away —
