...
Back to the guest list, and this time it is a bad one. Eleven lines, and a human at the door reads
ten of them as the same person:
...
Eleven strings. A `set` collapses **none** of them, because a set only merges values that are exactly
equal
([Day 8, 2.1](../../../day-08-containers/parts/02-sets-and-dicts/2.1-a-set-is-a-hash-table.md)).

That gap — between what a person sees and what `==` sees — is the whole job, and the plan names this
day's version of it: *normalising 500 messy titles with one chained expression.*

It is not one method call. It is a sequence of deliberate decisions, each of which throws away one
specific kind of difference, and each of which you have to be able to defend out loud. Strip the full
stop at the end? Yes. Strip the year in brackets? Probably — but then two genuinely different
conference versions become one record, and somebody has to decide whether that is what was wanted.
