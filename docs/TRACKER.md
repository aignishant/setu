...
> **Plan v2.0.0.** A day counts as *written* only when it has a hub **and** a non-empty `parts/` directory (Principle 16 · plan Part 11). The v1.0.0 single-file lessons were deleted rather than converted, so `days/` refills one rewritten day at a time.
