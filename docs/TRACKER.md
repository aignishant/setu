...
> **Plan v2.1.0.** A day counts as *written* only when it has a hub **and** a non-empty `parts/` directory (Principle 16 · plan Part 11). Folders are named for their subject — `days/day-NN-<slug>/parts/NN-<slug>/` — so this table and the file tree read the same way. The v1.0.0 single-file lessons were deleted rather than converted, so `days/` refills one rewritten day at a time.
