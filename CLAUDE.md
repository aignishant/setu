...
**Do not read the plan, the day map and the tracker whole to find out what a day covers.** Run
`./m brief N`. It projects that day's working set — its IDs with their full matrix rows, the phase
theme and gate, the neighbouring days' part lists — copied verbatim from those files. `days/INDEX.md`
is the same idea across all written parts: one row each, so you can find where an idea was already
taught and link to it instead of opening days to search. Both are generated by `./m tracker` and
must never be hand-edited. **Neither one replaces the plan's Part 11**, which is the depth contract
and is read in full every time a day is written — see the rule at the end of this file.
...
## Generated files vs. the contract (the token rule)

`./m brief N`, `days/INDEX.md`, `docs/TRACKER.md` and `docs/PINS_DS.md` are **projections**: every
line in them is copied from a source file, so they cannot invent a requirement the plan does not
contain. That is exactly why they may stand in for reading their sources — and why they are
regenerated, never hand-edited.

**A projection may replace a lookup. It may never replace a judgement.** The plan's Part 11, Part 1
and this file are prose that decides whether a day is good enough; they are read in full, and are
never summarised, compressed or projected to save tokens. If you are ever choosing between loading
fewer tokens and reading the depth contract, read the depth contract.
